...
_rdc_flags = ("--device-c", "-dc", "-rdc=true", "--relocatable-device-code=true")
_cudadevrt = None
...
def _run_cc(cmd, cwd, backend, log_stream=None):
    ...
    try:
        ...
        msg = (
            ...
            "return-code: {}\n"
            "stdout/stderr: \n"
            "{}".format(backend, e.cmd, e.returncode, e.output)
        )
        if backend == "nvcc":
        ...
        msg = "Failed to run `{0}` command. " "Check PATH environment variable: " + str(
            e
        )
...
def _get_cl_exe_dir() -> str | None:
    try:
        ...
        warnings.warn(f"Failed to find cl.exe with setuptools.msvc: {type(e)}: {e}")
    return None
...
def _get_cl_exe_dir_fallback() -> str | None:
    ...
    try:
        ...
        warnings.warn(f"Failed to find cl.exe with setuptools: {type(e)}: {e}")
    return None
...
def _get_extra_include_dir_opts():
    ...
    return tuple(
        f"-I{d}"
        for d in _environment._get_include_dir_from_conda_or_wheel(major, minor)
    )
...
def _get_arch_for_options_for_nvrtc(arch=None):
    ...
    if arch is None:
        arch = _get_arch()
    if not _use_ptx and int(arch) <= int(_get_max_compute_capability()):
...
def _get_cudadevrt_path():
    ...
    if not os.path.isfile(cudadevrt):
        raise RuntimeError(
            "Relocatable PTX code is requested, but cudadevrt " "is not found."
        )
    return cudadevrt
...
def _preprocess(source, options, arch, backend):
    if backend == "nvrtc":
        ...
        try:
            result = compile_using_nvcc(
                source, options, arch, "preprocess.cu", code_type="ptx"
            )
        except CompileException as e:
            dump = _get_bool_env_variable("CUPY_DUMP_CUDA_SOURCE_ON_ERROR", False)
            if dump:
    ...
    return "\n".join(x for x in result.decode().splitlines() if x.startswith("//"))
...
def _compile_module_with_cache(
    ...
    *,
    ...
    if enable_cooperative_groups:
        if runtime.is_hip:
            raise ValueError("Cooperative groups is not supported in HIP.")
    ...
    cache_in_memory = (
        _get_bool_env_variable("CUPY_CACHE_IN_MEMORY", False) and backend == "nvrtc"
    )
...
class CompileException(Exception):
    ...
    def __reduce__(self):
        return (
            type(self),
            (self._msg, self.source, self.name, self.options, self.backend),
        )
...
def _compile_with_cache_hip(
    ...
    if arch is None:
        ...
        arch = device.Device().compute_capability
    if use_converter:
        source = _convert_to_hip_source(
            source, extra_source, is_hiprtc=(backend == "hiprtc")
        )
