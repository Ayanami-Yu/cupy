...
def batched_gesv(a, b):
    ...
    if not (
        ...
        raise ValueError(
            "a must have (..., M, M) shape and b must have (..., M) " "or (..., M, K)"
        )
    ...
    if n > get_batched_gesv_limit():
        warnings.warn(
            "The matrix size ({}) exceeds the set limit ({})".format(
    ...
    a_array = cupy.arange(
        a.data.ptr, a.data.ptr + a_step * bs, a_step, dtype=cupy.uintp
    )
    ldb = n
    b_step = ldb * nrhs * b.itemsize
    b_array = cupy.arange(
        b.data.ptr, b.data.ptr + b_step * bs, b_step, dtype=cupy.uintp
    )
...
def _check_two_vectors(x, y):
    ...
    if x.size != y.size:
        raise ValueError(
            "x and y must be the same size (actual: {} and {})"
            "".format(x.size, y.size)
        )
    if x.dtype != y.dtype:
        raise TypeError(
            "x and y must be the same dtype (actual: {} and {})"
