...
class _OpsRegister:
    ...
    class _Op:
        def __init__(self, in_types, out_types, func):
            ...
            self.sig_str = (
                "".join(in_t.char for in_t in self.in_types)
                + "->"
                + "".join(out_t.char for out_t in self.out_types)
            )
    ...
    def _determine_from_signature(self, signature):
        ...
        if isinstance(signature, tuple):
            ...
            signature = (
                "".join(numpy.dtype(t).char for t in signature[:nin])
                + "->"
                + "".join(numpy.dtype(t).char for t in signature[nin : nin + nout])
            )
...
class _GUFunc:
    ...
    def __init__(self, func, signature, **kwargs):
        ...
        self._nout = (
            0 if not isinstance(output_coredimss, list) else len(output_coredimss)
        )
    ...
    def _get_args_transposed(self, args, input_axes, outs, output_axes):
        ...
        missing_dims = set()
        for i, (arg, iax, input_coredims, md) in enumerate(
            zip(args, input_axes, self._input_coredimss, self._min_dims)
        ):
            shape = arg.shape
            nds = len(shape)
            ...
            if nds < md:
                raise ValueError(
                    f"Input operand {i} does not have enough"
        ...
        args = [
            _manipulation.broadcast_to(
                a, shape + a.shape[-len(self._input_coredimss) :]
            )
            for a in args
        ]
        ...
        num_loopdims = [
            len(s) - len(cd) for s, cd in zip(input_shapes, self._input_coredimss)
        ]
        max_loopdims = max(num_loopdims) if num_loopdims else None
        core_input_shapes = [
            dict(zip(icd, s[n:]))
            for s, n, icd in zip(input_shapes, num_loopdims, self._input_coredimss)
        ]
        core_shapes = {}
    ...
    def _determine_order(self, args, order):
        if order.upper() in ("C", "K"):
            ...
            order = (
                "F"
                if all(
                    [a.flags.f_contiguous and not a.flags.c_contiguous for a in args]
                )
                else "C"
            )
            return order
    ...
    def __call__(self, *args, **kwargs):
        ...
        args, dimsizess, loop_output_dims, outs, m_dims = self._get_args_transposed(
        ...
        if outs is None:
            ...
            if outs[0].shape != out_shape:
                raise ValueError(
                    f"Invalid shape for out {outs[0].shape}" f" needs {out_shape}"
                )
