...
def histogram(x, bins=10, range=None, density=False, weights=None):
    ...
    if weights is None:
        ...
        for accelerator in _accelerator.get_routine_accelerators():
            ...
            if (
                accelerator == _accelerator.ACCELERATOR_CUB
                and x.size <= 0x7FFFFFFF
                and bin_edges.size <= 0x7FFFFFFF
            ):
                ...
                if numpy.issubdtype(x.dtype, numpy.integer):
                    ...
                    if bin_type == numpy.float16 and not common._is_fp16_supported():
        ...
        simple_weights = cupy.can_cast(weights.dtype, cupy.float64) or cupy.can_cast(
            weights.dtype, cupy.complex128
        )
...
_bincount_kernel = _core.ElementwiseKernel(
    "S x", "raw U bin", "atomicAdd(&bin[x], U(1))", "cupy_bincount_kernel"
)


_bincount_with_weight_kernel = _core.ElementwiseKernel(
    "S x, T w", "raw U bin", "atomicAdd(&bin[x], w)", "cupy_bincount_with_weight_kernel"
)
