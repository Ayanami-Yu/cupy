...
def _preload_library(lib):
    ...
    for filename in filenames:
        ...
        # "bin": cuDNN (Windows)
        libpath_cands = [
            os.path.join(cupy_cuda_lib_path, config["cuda"], lib, version, x, filename)
        ...
        if lib == "cutensor":
            ...
            min_pypi_version = config[lib]["min_pypi_version"]
            libpath_cands = (
                _get_nccl_from_wheel(min_pypi_version, config["cuda"]) + libpath_cands
            )
        ...
        for libpath in libpath_cands:
            ...
            try:
                ...
                msg = f"CuPy failed to preload library ({libpath}): " f"{e_type} ({e})"
                _log(msg)
                warnings.warn(msg)
        else:
...
def _find_compatible_wheel(
    ...
    is_compatible = actual[0] == expected[0] and (
        actual[1] > expected[1]
        or (actual[1] == expected[1] and actual[2] >= expected[2])
    )
    if not is_compatible:
        _log(
            f"{package_prefix} wheel package ({pkg}) incompatible: "
...
def _get_cutensor_from_wheel(version: str, cuda: str) -> list[str]:
    ...
    if sys.platform == "linux":
        shared_libs = [
            dist.locate_file(f'cutensor/lib/libcutensor.so.{version.split(".")[0]}'),
            dist.locate_file(f'cutensor/lib/libcutensorMg.so.{version.split(".")[0]}'),
        ]
    else:
        shared_libs = [
...
def _get_nccl_from_wheel(version: str, cuda: str) -> list[str]:
    ...
    if dist is None:
        return []
    shared_libs = [
        dist.locate_file(f'nvidia/nccl/lib/libnccl.so.{version.split(".")[0]}'),
    ]
    return [str(lib) for lib in shared_libs]
...
def _detect_duplicate_installation():
    ...
    installed_names = {
        d.metadata.get("Name", None) for d in importlib.metadata.distributions()
    }
  ...
  https://docs.cupy.dev/en/latest/install.html"""  # NOQA
    ...
    if sys.platform == "win32":
        try:
            ...
            msg += "\n\nThe cause could not be identified: " f"{type(e).__name__}: {e}"
