...
_dot_kernel = _core.ReductionKernel(
    "T x1, T x2", "T y", "x1 * x2", "a + b", "y = a", "0", "dot_product"
)
...
_legacy_sign = _core.create_ufunc(
    ...
    (
        ...
        "f->f",
        "d->d",
        ("F->F", _legacy_complex_sign),
        ("D->D", _legacy_complex_sign),
    ),
    "out0 = (in0 > 0) - (in0 < 0)",
)
...
"""
_float_maximum = (
    "out0 = (isnan(in0) | isnan(in1)) ? out0_type(NAN) : " "out0_type(max(in0, in1))"
)
...
_float_minimum = (
    "out0 = (isnan(in0) | isnan(in1)) ? out0_type(NAN) : " "out0_type(min(in0, in1))"
)
...
_nan_to_num = _core.create_ufunc(
    ...
    (
        ...
        "qqqq->q",
        "QQQQ->Q",
        ("eeee->e", "out0 = nan_to_num(in0, in1, in2, in3)"),
...
def interp(x, xp, fp, left=None, right=None, period=None):
    ...
    if not cupy.can_cast(x_dtype, cupy.float64):
        raise TypeError(
            ...
            " {} to {} according to the rule 'safe'".format(x_dtype, cupy.float64)
        )
