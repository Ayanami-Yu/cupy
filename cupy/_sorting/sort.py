...
def lexsort(keys):
    ...
    if not cupy.cuda.thrust.available:
        raise RuntimeError(
            "Thrust is needed to use cupy.lexsort. Please "
    ...
    if keys.ndim > 2:
        raise NotImplementedError(
            "Keys with the rank of three or more is not " "supported in lexsort"
        )
...
def msort(a):
    ...
    """
    warnings.warn(
        "msort is deprecated, use cupy.sort(a, axis=0) instead", DeprecationWarning
    )
