...
@pytest.mark.skipif(
    not cupy.cuda.cutensor.available, reason="The cuTENSOR routine is not enabled"
)
...
class TestNansumNanprodLong:
    ...
    def _numpy_nanprod_implemented(self):
        return self.func == "nanprod" and numpy.__version__ >= numpy.lib.NumpyVersion(
            "1.10.0"
        )
...
class TestGradient:
    ...
    def test_gradient_float16(self, xp):
        return self._gradient(
            xp, numpy.float16, self.shape, self.spacing, self.axis, self.edge_order
        )
...
class TestEdiff1d:
    ...
    def test_ediff1d_ed1(self, xp, dtype):
        a = testing.shaped_arange((2, 3, 4, 5), xp, dtype)
        return xp.ediff1d(
            a, to_begin=xp.array([-1], dtype=dtype), to_end=xp.array([0], dtype=dtype)
        )
