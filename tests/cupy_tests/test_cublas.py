...
class TestGer:
    ...
    def setUp(self):
        ...
        self.a = testing.shaped_random(
            self.shape, cupy, dtype=self.dtype, order=self.order
        )
...
class TestDgmm:
    ...
    def _setup(self, dtype, xdim=1):
        ...
        self.tol = self._tol[self.dtype.char.lower()]
        self.a = testing.shaped_random(
            self.shape, cupy, dtype=dtype, order=self.ordera, scale=1.0
        )
        if self.side == "L":
            xlen = self.shape[0]
        elif self.side == "R":
    ...
    def _check_dgmm_incx_minus_one_hip_skip_condition(self):
        return (
            ...
            self.shape,
            self.side,
        ) in self._dgmm_incx_minus_one_hip_skip_condition
