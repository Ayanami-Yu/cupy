...
class TestFromData(unittest.TestCase):
    ...
    def test_array_from_list_of_numpy_view(self, xp, dtype, src_order, dst_order):
        ...
        a = [
            (testing.shaped_arange((3, 8), numpy, dtype, src_order) + (24 * i))[:, ::2]
            for i in range(2)
        ]
        return xp.array(a, order=dst_order)
    ...
    def test_array_from_list_of_cupy_view(self, xp, dtype, src_order, dst_order):
        ...
        # create a list of view of ndarrays
        a = [
            (testing.shaped_arange((3, 8), xp, dtype, src_order) + (24 * i))[:, ::2]
            for i in range(2)
        ]
        return xp.array(a, order=dst_order)
...
class TestCudaArrayInterface(unittest.TestCase):
    ...
    def test_order(self, dtype):
        a = testing.shaped_arange((2, 3, 4), cupy, dtype)
        b = cupy.asarray(
            DummyObjectWithCudaArrayInterface(a, self.ver, self.strides), order="F"
        )
        assert b.flags.f_contiguous
        testing.assert_array_equal(a, b)
    ...
    def test_big_endian(self):
        ...
        b = DummyObjectWithCudaArrayInterface(
            (shape, strides, dtype.str, dtype.descr, data), self.ver, self.strides
        )
...
@testing.parameterize(
    *testing.product(
        {"ndmin": [0, 1, 2, 3], "copy": [True, False, None], "xp": [numpy, cupy]}
    )
)
