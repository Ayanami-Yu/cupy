...
class TestCheckCupyNumpyError(unittest.TestCase):
    ...
    def test_cupy_numpy_different_error(self):
        ...
        pattern = re.compile(self.tbs.get(cupy) + ".*" + self.tbs.get(numpy), re.S)
...
class TestNumPyCuPyEqual(
    unittest.TestCase, NumPyCuPyDecoratorBase, NumPyCuPyDecoratorBase2
):
...
@testing.parameterize({"decorator": "numpy_cupy_array_equal"})
...
@testing.parameterize({"decorator": "numpy_cupy_array_less"})
class TestNumPyCuPyLess(
    unittest.TestCase, NumPyCuPyDecoratorBase, NumPyCuPyDecoratorBase2
):
...
@testing.parameterize(*testing.product({"framework": ["unittest", "pytest"]}))
class TestSkip(unittest.TestCase):
    ...
    def test_array_almost_equal_nulp(self, xp):
        raise self._skip("Test for skip with @numpy_cupy_array_almost_equal_nulp")
        assert False
