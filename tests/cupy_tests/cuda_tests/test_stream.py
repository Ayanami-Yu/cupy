...
class TestStream(unittest.TestCase):
    ...
    @unittest.skipIf(cuda.runtime.is_hip, "HIP does not support launch_host_func")
    def test_launch_host_func(self):
        ...
        for i in range(N):
            numpy_array = cupy_arrays[i].get(stream=stream)
            stream.launch_host_func(lambda t: out.append(t[0]), (i, numpy_array))
...
class TestExternalStream(unittest.TestCase):
    ...
    def test_get_and_add_callback(self):
        ...
        for i in range(N):
            numpy_array = cupy_arrays[i].get(stream=stream)
            stream.add_callback(lambda _, __, t: out.append(t[0]), (i, numpy_array))
