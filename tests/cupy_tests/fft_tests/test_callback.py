...
def _set_load_cb(code, element, data_type, callback_type, aux_type=None):
    return string.Template(code).substitute(
        data_type=data_type, aux_type=aux_type, load_type=callback_type, element=element
    )
...
@pytest.mark.skipif(
    not sys.platform.startswith("linux"), reason="callbacks are only supported on Linux"
)
@pytest.mark.skipif(
...
class Test1dCallbacks:
    ...
    def _test_load_store_aux_helper(self, xp, dtype, fft_func):
        ...
        if fft_func in ("fft", "ifft"):
            if dtype == np.complex64:
                load_types = ("x.x", "cufftComplex", "cufftCallbackLoadC", "float")
                store_types = ("x.y", "cufftComplex", "cufftCallbackStoreC", "float")
            else:
                load_types = (
                    "x.x",
                    "cufftDoubleComplex",
            ...
            if dtype == np.float32:
                ...
                load_types = ("x", "cufftDoubleReal", "cufftCallbackLoadD", "double")
                store_types = (
                    "x.y",
                    "cufftDoubleComplex",
                    "cufftCallbackStoreZ",
            ...
            if dtype == np.complex64:
                ...
                store_types = ("x", "cufftDoubleReal", "cufftCallbackStoreD", "double")
        cb_load = _set_load_cb(load_code, *load_types)
        cb_store = _set_store_cb(store_code, *store_types)
