...
@testing.parameterize(
    *augment_einsum_testcases(
        ...
        # tensordot
        {"shape_a": (3, 4, 2), "shape_b": (4, 3, 2), "subscripts": "ijk, jil -> kl"},
        {"shape_a": (3, 4, 2), "shape_b": (4, 2, 3), "subscripts": "i...,...k->ki..."},
        {"shape_a": (3, 4, 2), "shape_b": (4, 3, 2), "subscripts": "ij...,ji...->i..."},
        # trace and tensordot and diagonal
        {"shape_a": (2, 3, 2, 4), "shape_b": (3, 2, 2), "subscripts": "ijil,jkk->kj"},
        {
            "shape_a": (2, 4, 2, 3),
            "shape_b": (3, 2, 4),
        ...
        {"shape_a": (4, 3), "shape_b": (3, 2), "subscripts": "ik...,...kj->i...j"},
    ...
    def test_einsum_ternary(self, xp, dtype_a, dtype_b, dtype_c):
        ...
        if xp is not numpy:  # Avoid numpy issues #11059, #11060
            for optimize in [
                ...
                optimized_out = xp.einsum(self.subscripts, a, b, c, optimize=optimize)
                testing.assert_allclose(optimized_out, out)
        return out
...
class TestEinSumLarge:
    ...
    def test_einsum(self, xp, shapes):
        arrays = [testing.shaped_random(shape, xp, float, scale=1) for shape in shapes]
        ...
        with warnings.catch_warnings(record=True) as ws:
            ...
            if xp is not numpy and isinstance(self.opt, tuple):  # with memory limit
                for w in ws:
