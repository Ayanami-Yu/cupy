...
class _TestRawBase:
    ...
    @unittest.skipIf(
        cupy.cuda.runtime.is_hip, "ROCm/HIP does not support dynamic parallelism"
    )
    ...
    def test_dynamical_parallelism_compile_failure(self):
        ...
        ker = cupy.RawKernel(
            _test_source4, "test_kernel", backend=self.backend, jitify=self.jitify
        )
        N = 10
        inner_chunk = 2
        x = cupy.zeros((N,), dtype=cupy.float32)
    ...
    def test_grid_sync_rawmodule(self):
        ...
        with use_temporary_cache_dir():
            mod_grid_sync = cupy.RawModule(
                code=_test_grid_sync, backend="nvcc", enable_cooperative_groups=True
            )
            x1 = cupy.arange(n**2, dtype="float32").reshape(n, n)
            x2 = cupy.ones((n, n), dtype="float32")
            y = cupy.zeros((n, n), dtype="float32")
...
class TestRawPicklable(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if self.raw == "ker":
            ...
            self.mod = cupy.RawModule(
                code=_test_source1, backend="nvcc", enable_cooperative_groups=True
            )
    ...
    def test_raw_picklable(self):
        ...
        s = subprocess.run(
            [sys.executable, "TestRawPicklable.py"] + test_args, cwd=self.temp_dir
        )
