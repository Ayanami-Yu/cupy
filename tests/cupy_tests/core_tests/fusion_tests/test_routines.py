...
class TestFusionBitwiseBinary(FusionBinaryUfuncTestBase):
    ...
    def test_bitwise(self, xp, dtype1, dtype2):
        def impl(x, y):
            if (x.dtype == "uint64" and y.dtype.kind == "i") or (
                y.dtype == "uint64" and x.dtype.kind == "i"
            ):
                # Skip TypeError case.
                return
...
@testing.parameterize(
    *testing.product({"func": ["deg2rad", "rad2deg", "degrees", "radians"]})
)
...
@testing.parameterize(
    *testing.product({"func": ["add", "subtract", "multiply", "power"]})
)
...
@testing.parameterize(
    *testing.product({"func": ["sum", "prod", "amax", "amin", "max", "min"]})
)
