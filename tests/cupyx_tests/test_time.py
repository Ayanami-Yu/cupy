...
class TestRepeat(unittest.TestCase):
    ...
    def test_cpu_routine(self):
        with mock.patch(
            "time.perf_counter", mock.Mock(side_effect=[2.4, 3.8, 3.8] * 10)
        ):
    ...
    def test_repeat_max_duration(self):
        with mock.patch(
            "time.perf_counter", mock.Mock(side_effect=[1.0, 2.0, 2.0] * 6)
        ):
            with mock.patch("cupy.cuda.get_elapsed_time", mock.Mock(return_value=2500)):
                mock_func = mock.Mock()
                mock_func.__name__ = "test_name_xxx"
    ...
    def test_repeat_kwargs(self):
        ...
        cupyx.time.repeat(cupy.nonzero, kwargs={"a": x}, n_repeat=1, n_warmup=1)
...
class TestPerfCaseResult(unittest.TestCase):
    ...
    def test_single_show_gpu(self):
        ...
        perf = cupyx.time._PerfCaseResult("test_name_xxx", times, (0,))
        assert str(perf) == (
            "test_name_xxx       :    CPU:     5.400 us " "    GPU-0:     6.400 us"
        )
