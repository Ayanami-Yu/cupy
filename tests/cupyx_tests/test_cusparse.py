...
class TestCsrmv:
    ...
    def test_csrmv(self):
        ...
        y = cusparse.csrmv(a, x, alpha=self.alpha, transa=self.transa)
    ...
    def test_csrmvEx_with_y(self):
        ...
        x = cupy.array(self.x, order="f")
        y = cupy.array(self.y, order="f")
        z = cusparse.csrmvEx(a, x, y=y, alpha=self.alpha, beta=self.beta)
        expect = self.alpha * self.op_a.dot(self.x) + self.beta * self.y
        assert y is z
        testing.assert_array_almost_equal(y, expect)
...
class TestSpmv:
    ...
    def setUp(self):
        m, n = self.shape
        self.op_a = scipy.sparse.random(
            m, n, density=0.5, format=self.format, dtype=self.dtype
        )
        if self.transa:
            self.a = self.op_a.T
        else:
    ...
    def test_spmv(self):
        ...
        if runtime.is_hip:
            if (
                (self.format == "csr" and self.transa is True)
...
class TestErrorSpmv:
    ...
    def setUp(self):
        m, n = 2, 3
        self.a = scipy.sparse.random(m, n, density=0.5, dtype=self.dtype)
        self.x = numpy.random.uniform(-1, 1, n).astype(self.dtype)
        self.y = numpy.random.uniform(-1, 1, m).astype(self.dtype)
...
class TestSpmm:
    ...
    def setUp(self):
        ...
        m, n, k = self.dims
        self.op_a = scipy.sparse.random(
            m, k, density=0.5, format=self.format, dtype=self.dtype
        )
        if self.transa:
            self.a = self.op_a.T
        else:
...
class TestCsrsm2:
    ...
    def _setup(self, dtype):
        ...
        b = numpy.arange(1, numpy.prod(b_shape) + 1, dtype=dtype).reshape(b_shape)
    ...
    def test_csrsm2(self, dtype):
        ...
        if runtime.is_hip:
            if self.transa == "H" or (
                driver.get_build_version() < 400
                and (
                    (self.format == "csc" and self.transa == "N")
        ...
        if (
            self.format == "csc"
            and numpy.dtype(dtype).char in "FD"
            and self.transa == "H"
        ):
...
class TestCsrilu02:
    ...
    def _make_matrix(self, dtype):
        ...
        a = testing.shaped_random((self.n, self.n), cupy, dtype=dtype, scale=0.9) + 0.1
...
class TestSparseMatrixConversion:
    ...
    def test_sparseToDense(self, dtype):
        ...
        x = scipy.sparse.random(
            m, n, density=self.density, format=self.format, dtype=dtype
        )
        if self.format == "csr":
...
class TestSpsm:
    ...
    def _make_matrix(self, dtype, m, lower, unit_diag, format):
        ...
        a = scipy.sparse.random(m, m, density=0.5, format=format, dtype=dtype)
    ...
    def test_spsm(self, lower, unit_diag, transa, b_order, dtype, format):
        ...
        c = cusparse.spsm(
            a, b, alpha=self.alpha, lower=lower, unit_diag=unit_diag, transa=transa
        )
