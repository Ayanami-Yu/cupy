...
class TestCuTensor:
    ...
    def setUp(self):
        self.a = testing.shaped_random((20, 40, 30), cupy, self.dtype, seed=0)
        self.b = testing.shaped_random((40, 30, 20), cupy, self.dtype, seed=1)
        self.c = testing.shaped_random((30, 20, 40), cupy, self.dtype, seed=2)
        self.mode_a = ("y", "z", "x")
        self.mode_b = ("z", "x", "y")
        self.mode_c = ("x", "y", "z")
        self.alpha = 1.1
    ...
    def test_elementwise_binary(self):
        d = cutensor.elementwise_binary(
            self.alpha, self.a, self.mode_a, self.gamma, self.c, self.mode_c
        )

        assert d.dtype == self.dtype

        testing.assert_allclose(
            self.alpha * self.a_transposed + self.gamma * self.c_transposed,
            d,
            rtol=self.tol,
            atol=self.tol,
        )
    ...
    def test_elementwise_binary_out(self):
        ...
        d = cutensor.elementwise_binary(
            self.alpha, self.a, self.mode_a, self.gamma, self.c, self.mode_c, out=out
        )
    ...
    def test_reduction(self):
        ...
        d = cutensor.reduction(self.alpha, self.a, self.mode_a, self.beta, c, ("x",))

        assert c is d
        testing.assert_allclose(
            self.alpha * self.a_transposed.sum(axis=(1, 2)) + self.beta * c_orig,
            d,
            rtol=self.tol,
            atol=self.tol,
        )
...
class TestCuTensorDescriptor:
    ...
    @pytest.fixture(autouse=True)
    def setUp(self):
        self.a = testing.shaped_random((20, 40, 30), cupy, numpy.float32, seed=0)
        self.b = testing.shaped_random((40, 30, 20), cupy, numpy.float32, seed=1)
        self.c = testing.shaped_random((30, 20, 40), cupy, numpy.float32, seed=2)
        self.mode_a = ("y", "z", "x")
        self.mode_b = ("z", "x", "y")
        self.mode_c = ("x", "y", "z")
        self.alpha = 1.1
    ...
    def test_elementwise_binary(self):
        ...
        testing.assert_allclose(
            self.alpha
            * (1 / (1 + cupy.exp(-self.a_transposed)))
            * self.gamma
            * cupy.abs(self.c),
            d,
            rtol=1e-6,
...
class TestCuTensorContraction:
    ...
    def test_contraction(self):
        ...
        cutensor.contraction(
            self.alpha, self.a, mode_a, self.b, mode_b, self.beta, self.c, mode_c
        )
        cupy.testing.assert_allclose(self.c, self.c_ref, rtol=self.tol, atol=self.tol)
...
class TestCuTensorIncontiguous:
    ...
    def test_contraction(self):
        ...
        self.c = testing.shaped_random((d, a), cupy, dtype=self.dtype, order=self.order)
        delta = 7
        c_ref = self.c.copy()
        c_ref = cutensor.contraction(
            self.alpha, self.a, mode_a, self.b, mode_b, self.beta, c_ref, mode_c
        )
        for a0 in range(0, a, delta):
            for d0 in range(0, d, delta):
                cutensor.contraction(
                    ...
                    mode_b,
                    self.beta,
                    self.c[d0 : d0 + delta, a0 : a0 + delta],
                    mode_c,
                )
    ...
    def test_reduction(self):
        ...
        self.c = testing.shaped_random((b,), cupy, dtype=self.dtype, order=self.order)

        c_ref = self.c.copy()
        c_ref = cutensor.reduction(self.alpha, self.a, mode_a, self.beta, c_ref, mode_c)
        delta = 7
    ...
    def test_elementwise_binary(self):
        ...
        c_ref = self.c.copy()
        c_ref = cutensor.elementwise_binary(
            self.alpha, self.a, mode_a, self.beta, c_ref, mode_c
        )
        delta = 7
        for b0 in range(0, b, delta):
    ...
    def test_elementwise_trinary(self):
        ...
        for gamma in [0.0, 1.0]:
            c_ref = self.c.copy()
            c_ref = cutensor.elementwise_trinary(
                self.alpha,
                self.a,
                mode_a,
                self.beta,
...
class TestCuTensorMg:
    ...
    def test_contraction(self):
        ...
        if self.dtype == "e":
            ...
            self.a = testing.shaped_random((n, n, n, n), numpy, dtype=self.dtype)
        self.b = testing.shaped_random((n, n, n, n), cupy, dtype=self.dtype)
        ...
        cupy.testing.assert_allclose(self.c, c_ref, rtol=self.tol, atol=self.tol)
    ...
    def test_copy(self):
        ...
        cupy.cuda.Device(0).synchronize()
        cupy.testing.assert_allclose(
            self.b.get(), self.a.transpose((2, 0, 1, 3)), rtol=self.tol, atol=self.tol
        )
