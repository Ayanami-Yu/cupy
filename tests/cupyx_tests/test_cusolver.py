...
class TestGesvdj:
    ...
    def test_gesvdj(self):
        ...
        u, s, v = cusolver.gesvdj(
            a, full_matrices=self.full_matrices, overwrite_a=self.overwrite_a
        )
    ...
    def test_gesvdj_no_uv(self):
        ...
        expect = numpy.linalg.svd(
            self.a, full_matrices=self.full_matrices, compute_uv=False
        )
        if self.dtype in (numpy.float32, numpy.complex64):
            decimal = 5
        else:
...
class TestSyevj:
    ...
    def setUp(self):
        ...
        if self.dtype in (numpy.complex64, numpy.complex128):
            ...
            self.a = numpy.array([[1, 2, 3], [2, 5, 4], [3, 4, 9]], dtype=self.dtype)
    ...
    def test_syevj(self):
        ...
        testing.assert_allclose(self.a.dot(v.get()), w * v, rtol=1e-3, atol=1e-4)
...
class TestCsrlsvqr:
    ...
    def test_csrlsvqr(self, dtype):
        ...
        x = cupyx.cusolver.csrlsvqr(sp_a, cp_b, tol=self.tol, reorder=self.reorder)
