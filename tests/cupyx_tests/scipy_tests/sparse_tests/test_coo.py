...
class TestCooMatrixInit:
    ...
    def test_data_invalid(self):
        with pytest.raises(ValueError):
            sparse.coo_matrix(
                ("invalid", (self.row(cupy), self.col(cupy))), shape=self.shape
            )
    ...
    def test_row_invalid(self):
        with pytest.raises(ValueError):
            sparse.coo_matrix(
                (self.data(cupy), ("invalid", self.col(cupy))), shape=self.shape
            )
    ...
    def test_col_invalid(self):
        with pytest.raises(ValueError):
            sparse.coo_matrix(
                (self.data(cupy), (self.row(cupy), "invalid")), shape=self.shape
            )
...
class TestCooMatrixScipyComparison:
    ...
    @pytest.mark.xfail(
        scipy_available
        and numpy.lib.NumpyVersion(scipy.__version__) >= "1.8.0rc1"
        and numpy.lib.NumpyVersion(scipy.__version__) <= "1.9.0rc1",
...
class TestIsspmatrixCoo:
    ...
    def test_coo(self):
        x = sparse.coo_matrix(
            (cupy.array([0], "f"), (cupy.array([0], "i"), cupy.array([0], "i"))),
            shape=(1, 1),
            dtype="f",
        )
