...
class TestVectorNorm:
    ...
    @testing.numpy_cupy_allclose(
        rtol=1e-3, atol=1e-4, sp_name="sp", accept_error=(ValueError,)
    )
    def test_vector_norm(self, xp, sp):
        if runtime.is_hip:
            if self.axis in (0, (-2,)) and self.ord in (-2, -1, 0, 1, 2, None):
                pytest.xfail("csc spmv is buggy")
...
class TestEigsh:
    ...
    def _test_eigsh(self, a, xp, sp):
        ret = sp.linalg.eigsh(
            a, k=self.k, which=self.which, return_eigenvectors=self.return_eigenvectors
        )
...
class TestSvds:
    ...
    def _test_svds(self, a, xp, sp):
        ret = sp.linalg.svds(a, k=self.k, return_singular_vectors=self.return_vectors)
...
class TestGmres:
    ...
    def _make_matrix(self, dtype, xp):
        ...
        a[mask > self.density] = 0
        diag = xp.diag(
            testing.shaped_random((self.n,), xp, dtype=dtype.char.lower(), scale=1) + 1
        )
        a[diag > 0] = 0
        a = a + diag
        M = None
    ...
    def _test_gmres(self, dtype, xp, sp, a, M):
        ...
        if self.atol == "select-by-dtype":
            atol = self._atol[dtype.char.lower()]
        return sp.linalg.gmres(a, b, x0=x0, restart=self.restart, M=M, atol=atol)
...
def skip_HIP_spMM_error(outer=()):
    def decorator(impl):
        ...
        def test_func(self, *args, **kw):
            if (
                runtime.is_hip
                and self.inner_modification == "sparse"
...
class TestLinearOperator:
    ...
    @skip_HIP_spMM_error(outer=("transpose", "hermitian"))
    @testing.numpy_cupy_allclose(sp_name="sp", rtol=1e-6, contiguous_check=False)
...
@pytest.mark.skipif(
    not cusparse.check_availability("csrsm2"), reason="no working implementation"
)
class TestSpsolveTriangular:
    ...
    def test_invalid_cases(self):
        dtype = "float64"
        if not (
            self.lower and self.unit_diagonal and self.nrhs == 4 and self.order == "C"
        ):
            pytest.skip()
...
class TestCsrlsvqr:
    ...
    def test_csrlsvqr(self, dtype):
        ...
        cp_b = cupy.array(b)
        x = cusolver.csrlsvqr(sp_a, cp_b, tol=self.tol, reorder=self.reorder)
...
def _eigen_vec_transform(block_vec, xp):
    ...
    of eigen vectors"""
    direction = testing.shaped_random((block_vec.shape[0], 1), xp=xp, seed=123)
...
@testing.with_requires("scipy>=1.4")
@pytest.mark.skipif(
    runtime.is_hip and driver.get_build_version() < 402, reason="syevj not available"
)
...
class TestLOBPCG:
    ...
    def _generate_input_for_elastic_rod(self, n, xp):
        ...
        k = E * S / le
        A = k * (
            xp.diag(xp.r_[2.0 * xp.ones(n - 1), 1])
            - xp.diag(xp.ones(n - 1), 1)
            - xp.diag(xp.ones(n - 1), -1)
        )
        B = mass * (
            xp.diag(xp.r_[4.0 * xp.ones(n - 1), 2])
            + xp.diag(xp.ones(n - 1), 1)
            + xp.diag(xp.ones(n - 1), -1)
        )
        return A, B
    ...
    def test_small_generate_input_for_elastic_rod(self, xp, sp):
        ...
        eigvals, eigvecs = sp.linalg.lobpcg(
            A, X, B=B, tol=1e-5, maxiter=30, largest=False
        )
    ...
    def test_diagonal(self, xp, sp):
        ...
        eigvals, vecs = sp.linalg.lobpcg(
            A, X, B, M=M, Y=Y, tol=1e-4, maxiter=40, largest=False
        )
    ...
    def _generate_approximate_X_for_fiedler(self, n, p, xp):
        fiedler_guess = xp.concatenate((xp.ones(n // 2), -xp.ones(n - n // 2)))
    ...
    def _verbosity_helper(self, xp, sp):
        ...
        with contextlib.redirect_stdout(saved_stdout):
            _, _ = sp.linalg.lobpcg(
                A, X, B=B, tol=1e-5, maxiter=30, largest=False, verbosityLevel=9
            )
    ...
    @testing.numpy_cupy_allclose(
        rtol=1e-5,
        atol=5e-3 if runtime.is_hip else 1e-3,
        sp_name="sp",
        contiguous_check=False,
    )
    def test_random_initial_float32(self, xp, sp):
        ...
        eigvals, vecs = sp.linalg.lobpcg(A, X, tol=1e-3, maxiter=50, verbosityLevel=1)
        return eigvals, _eigen_vec_transform(vecs, xp)
    ...
    def test_maxit_None(self):
        ...
        w_np, _ = scipy.sparse.linalg.lobpcg(A_np, X_np, tol=1e-8, maxiter=None)
...
class TestLOBPCGForDiagInput:
    ...
    def setUp(self):
        if runtime.is_hip:
            ...
            if (
                (self.A_sparsity is True)
                or (self.B_sparsity is True)
    ...
    def test_diagonal_data_types(self, xp, sp):
        ...
        if self.preconditioner_dtype is not None:
            M = sp.diags([1.0 / vals], [0], (n, n), format=self.sparse_format)
            ...
            M_LO = sp.linalg.LinearOperator(
                matvec=fun,
                matmat=fun,
                shape=(n, n),
                dtype=xp.dtype(self.preconditioner_dtype),
            )  # NOQA

        # Cannot be sparse array
        X = testing.shaped_random(
            (n, m), xp=xp, dtype=xp.dtype(self.X_dtype), seed=1234
        )
        ...
        eigvals, eigvecs = sp.linalg.lobpcg(
            A, X, B=B, M=M_LO, Y=Y, tol=1e-4, maxiter=100, largest=False
        )
        return eigvals, _eigen_vec_transform(eigvecs, xp)
...
@testing.parameterize(
    *testing.product(
        {"format": ["csr", "csc", "coo"], "nrhs": [None, 1, 4], "order": ["C", "F"]}
    )
)
@testing.with_requires("scipy")
@pytest.mark.skipif(
...
class TestMinres:
    ...
    def test_invalid(self):
        if not (
            self.x0 is None and self.M is None and self.use_linear_operator is False
        ):
            pytest.skip()
        for xp, sp in ((numpy, scipy.sparse), (cupy, sparse)):
            a, M = self._make_matrix(xp)
