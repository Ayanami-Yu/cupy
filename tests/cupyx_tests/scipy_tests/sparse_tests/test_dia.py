...
class TestDiaMatrixInit(unittest.TestCase):
    ...
    def test_scipy_sparse(self):
        s_h = scipy.sparse.dia_matrix(
            (self.data(numpy), self.offsets(numpy)), shape=self.shape
        )
...
class TestIsspmatrixDia(unittest.TestCase):
    ...
    def test_dia(self):
        x = sparse.dia_matrix(
            (cupy.array([], "f"), cupy.array([0], "i")), shape=(0, 0), dtype="f"
        )
