...
class FilterTestCaseBase:
    ...
    ARGS_PARAMS = ("rank", "percentile", "derivative", "derivative2", "function")
...
class TestFilter(FilterTestCaseBase):
    ...
    def _hip_skip_invalid_condition(self):
        ...
        if self.mode == "mirror":
            ...
            if self.footprint and self.ksize == 3 and self.shape == (3, 4, 5):
                pytest.xfail("ROCm/HIP may have a bug")

    @testing.numpy_cupy_allclose(atol=1e-5, rtol=1e-5, scipy_name="scp")
    def test_filter(self, xp, scp):
        if self.dtype == numpy.uint8 and platform.machine() == "aarch64":
            pytest.skip("aarch64 scipy does not match cupy/x86 see Scipy #20158")
...
class TestFilterFast(FilterTestCaseBase):
    ...
    def _hip_skip_invalid_condition(self):
        ...
        if self.ksize != 3:
            return
        invalids = [(25, (1, 3, 4, 5)), (50, (3, 4, 5)), (-25, (1, 3, 4, 5))]
...
rms_red = cupy.ReductionKernel(
    "X x", "Y y", "x*x", "a + b", "y = a/_in_ind.size()", "0", "rms"
)
...
lt_red = cupy.ReductionKernel(
    "X x",
    "Y y",
    "_raw_x[_in_ind.size()/2]>x",
    "a + b",
    "y = a",
    "0",
    "lt",
...
class TestGenericFilter(FilterTestCaseBase):
    ...
    def get_func_or_kernel(self, xp):
        return self._func_or_kernels[self.func_or_kernel[1 if xp == numpy else 0]]
...
class TestWeightComplexDtype(FilterTestCaseBase):
    ...
    def _skip_noncomplex(self):
        if not (
            numpy.dtype(self.dtype).kind == "c" or numpy.dtype(self.wdtype).kind == "c"
        ):
...
class TestSpecialWeightCases(FilterTestCaseBase):
    ...
    @testing.numpy_cupy_allclose(
        atol=1e-5, rtol=1e-5, scipy_name="scp", accept_error=RuntimeError
    )
    ...
    @testing.numpy_cupy_allclose(
        atol=1e-5, rtol=1e-5, scipy_name="scp", accept_error=(RuntimeError, ValueError)
    )
