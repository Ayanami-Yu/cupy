...
class TestDistanceTransform:
    ...
    def test_distance_transform_edt_basic(
        self, shape, sampling, return_distances, return_indices, density, block_params
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (1, 3, 1),
            (1, 5, 1),
            (1, 7, 1),  # 2nd element must be power of 2
    ...
    def test_distance_transform_inplace_distance_errors(self, ndim):
        ...
        with pytest.raises(RuntimeError):
            distances = cupy.empty(img.shape, dtype=cupy.float64)
            dt_func(
                img, distances=distances, return_distances=False, return_indices=True
            )
    ...
    @pytest.mark.parametrize(
        "dtype",
        [cupy.int16, cupy.uint16, cupy.uint32, cupy.int32, cupy.uint64, cupy.int64],
    )
    @pytest.mark.parametrize("return_distances", [False, True])
    def test_distance_transform_inplace_indices(
        ...
        if sampling == "iso":
            ...
            sampling = tuple(range(1, ndim + 1))
        common_kwargs = dict(
            sampling=sampling, return_distances=return_distances, return_indices=True
        )
