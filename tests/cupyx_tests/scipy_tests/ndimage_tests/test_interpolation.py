...
class TestMapCoordinates:
    ...
    def _map_coordinates(self, xp, scp, a, coordinates):
        ...
        if self.output == "empty":
            ...
            return_value = map_coordinates(
                a, coordinates, output, self.order, self.mode, self.cval, self.prefilter
            )
    ...
    def test_map_coordinates_int(self, xp, scp, dtype):
        ...
        float_out = (
            self._map_coordinates(xp, scp, a.astype(xp.float64), coordinates) % 1
        )
...
class TestAffineExceptions:
    ...
    def test_invalid_texture_arguments(self):
        ...
        for dt in [cupy.float16, cupy.float64, cupy.int32, cupy.int64]:
            with pytest.raises(ValueError):
                aft(
                    cupy.ones((8, 8), dtype=dt),
                    cupy.eye(3, dtype=cupy.float32),
                    texture_memory=True,
                )
        # wrong matrix shape
...
class TestAffineTransformTextureMemory:
    ...
    def _2d_rotation_matrix(self, theta, rotation_center):
        ...
        m = numpy.array(
            [[1, 0, rotation_center[0]], [0, 1, rotation_center[1]], [0, 0, 1]],
            numpy.float32,
        )
    ...
    def _3d_rotation_matrix(self, theta, rotation_center):
        ...
        m = numpy.dot(
            m,
            numpy.array(
                [[1, 0, 0, 0], [0, c, -s, 0], [0, s, c, 0], [0, 0, 0, 1]], numpy.float32
            ),
        )
        m = numpy.dot(
            m,
    ...
    def test_affine_transform_texture_memory(self, xp, scp):
        ...
        if self.output == "empty":
            ...
            if self.output_shape:
                return pytest.skip(
                    "This combination is tested in " "test_invalid_texture_arguments"
                )
        else:
            output = self.output
...
class TestRotate:
    ...
    def _hip_skip_invalid_condition(self):
        if runtime.is_hip:
            ...
            if (
                self.angle == -15
                and self.mode in ["nearest", "grid-wrap", "reflect", "grid-mirror"]
                and self.order == 3
            ):
                pytest.xfail("ROCm/HIP may have a bug")
...
class TestRotateOpenCV:
    ...
    def test_rotate_opencv(self, xp, dtype):
        a = testing.shaped_random((100, 100), xp, dtype)
        if xp == cupy:
            return cupyx.scipy.ndimage.rotate(
...
class TestShift:
    ...
    def _shift(self, xp, scp, a):
        ...
        if self.output == "empty":
            output = xp.empty_like(a)
            return_value = shift(
                a, self.shift, output, self.order, self.mode, self.cval, self.prefilter
            )
            assert return_value is None or return_value is output
            return output
        else:
...
class TestInterpolationInvalidCval:
    ...
    def test_rotate(self, dtype):
        ...
        if _util._is_integer_output(output, a) and self.mode == "constant":
            ...
            rotate(
                a, 0, output=output, order=self.order, mode=self.mode, cval=self.cval
            )
...
class TestZoom:
    ...
    def _zoom(self, xp, scp, a):
        ...
        if self.output == "empty":
            output = zoom(
                a, self.zoom, None, self.order, self.mode, self.cval, self.prefilter
            )
            return_value = zoom(
                a, self.zoom, output, self.order, self.mode, self.cval, self.prefilter
            )
            assert return_value is None or return_value is output
            return output
        else:
...
class TestZoomOutputSize1:
    ...
    def test_zoom_output_size1(self, xp, scp, dtype):
        ...
        x[1, 1, 1] = 1
        return scp.ndimage.zoom(
            x, self.zoom, order=self.order, mode=self.mode, grid_mode=self.grid_mode
        )
    ...
    def test_zoom_opencv_output_size1(self, xp, dtype):
        ...
        if xp == cupy:
            return cupyx.scipy.ndimage.zoom(
                a, (self.zoom, self.zoom, 1), order=1, mode="opencv"
            )
        else:
...
class TestSplineFilter1d:
    ...
    def test_spline_filter1d(self, xp, scp):
        ...
        return scp.ndimage.spline_filter1d(
            x, order=self.order, axis=self.axis, output=self.output, mode=self.mode
        )
    ...
    def test_spline_filter1d_output(self, xp, scp, array_order):
        ...
        x = testing.shaped_random(
            (16, 12, 11), dtype=self.dtype, xp=xp, order=array_order
        )
        output = xp.empty(x.shape, dtype=self.output, order=array_order)
        scp.ndimage.spline_filter1d(
            x, order=self.order, axis=self.axis, output=output, mode=self.mode
        )
        return output
...
class TestSplineFilter:
    ...
    def test_spline_filter(self, xp, scp):
        ...
        if self.order < 2:
            with pytest.raises(RuntimeError):
                scp.ndimage.spline_filter(
                    x, order=self.order, output=self.output, mode=self.mode
                )
            return xp.asarray([])
        return scp.ndimage.spline_filter(
            x, order=self.order, output=self.output, mode=self.mode
        )
    ...
    def test_spline_filter_with_output(self, xp, scp, array_order):
        ...
        if self.order < 2:
            with pytest.raises(RuntimeError):
                scp.ndimage.spline_filter(
                    x, order=self.order, output=output, mode=self.mode
                )
            return xp.asarray([])
