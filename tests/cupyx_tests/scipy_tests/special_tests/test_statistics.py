...
class TestSpecial(_TestBase):
    ...
    @testing.for_dtypes(["e", "f", "d"])
    @testing.numpy_cupy_allclose(atol=atol_low, rtol=rtol_low, scipy_name="scp")
    ...
    @pytest.mark.parametrize("inverse", [False, True], ids=["boxcox", "inv_boxcox"])
    @testing.for_dtypes(["e", "f", "d"])
    @testing.numpy_cupy_allclose(atol=atol, rtol=rtol, scipy_name="scp")
...
class _TestDistributionsBase:
    ...
    def _test_scalar(self, function, args, expected, rtol=1e-12, atol=1e-12):
        ...
        testing.assert_allclose(function(*args), expected, rtol=rtol, atol=atol)
...
class TestTwoArgumentDistribution(_TestDistributionsBase):
    ...
    @testing.with_requires("scipy>=1.16")
    @pytest.mark.skipif(
        cupy.cuda.runtime.is_hip, reason="avoid failures observed on HIP"
    )
    @pytest.mark.parametrize(
        "function", ["chdtr", "chdtrc", "chdtri", "pdtr", "pdtrc", "pdtri"]
    )
...
class TestThreeArgumentDistributions(_TestDistributionsBase):
    ...
    @pytest.mark.parametrize("function", ["fdtr", "fdtrc", "fdtri", "gdtr", "gdtrc"])
    @testing.for_float_dtypes()
    @testing.numpy_cupy_allclose(atol=1e-5, rtol=1e-5, scipy_name="scp")
