...
class TestDelaunay:
    ...
    @pytest.mark.parametrize("dtype", [cupy.float64, cupy.float32, cupy.float16])
    ...
    @pytest.mark.parametrize("dataset", [pathological_data_1, pathological_data_2])
    @testing.numpy_cupy_allclose(scipy_name="scp")
    ...
    def test_find_simplex(self, point, xp, scp):
        points = xp.array(
            [(0, 0), (0, 1), (1, 1), (1, 0), (0.5, 0.5)], dtype=xp.float64
        )
        tri = scp.spatial.Delaunay(points)
