...
@pytest.mark.skipif(
    not scipy_available or not cuvs_available, reason="requires scipy and pylibraft"
)
class TestCdist(unittest.TestCase):
    ...
    def test_cdist_(self, xp, scp):
        ...
        if self.metric == "minkowski":
            out = scp.spatial.distance.cdist(a, b, metric=self.metric, p=self.p).astype(
                self.dtype
            )
        else:
            out = scp.spatial.distance.cdist(a, b, metric=self.metric).astype(
                self.dtype
            )
...
@pytest.mark.skipif(cupy.cuda.runtime.is_hip, reason="tests for CUDA only")
@pytest.mark.skipif(
    not scipy_available or not cuvs_available, reason="requires scipy and cuvs"
)
class TestPdist:

    def _make_matrix(self, xp, dtype, order):
        shape = (self.rows, self.cols)
        out = testing.shaped_random(shape, xp, dtype=dtype, scale=1, order=order)
        return out
