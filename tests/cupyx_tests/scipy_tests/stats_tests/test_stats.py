...
class TestTrim:
    ...
    @testing.numpy_cupy_allclose(scipy_name="scp", rtol=1e-6, contiguous_check=False)
    @pytest.mark.parametrize("shape", [(24,), (6, 4), (6, 4, 3), (4, 6)])
    def test_base(self, xp, scp, dtype, order, shape):
        a = testing.shaped_random(shape, xp=xp, dtype=dtype, order=order, scale=100)
        return scp.stats.trim_mean(a, 2 / 6.0)
    ...
    def test_zero_dim_axis_none(self, xp, scp, dtype):
        a = xp.array(0, dtype=dtype)
        return scp.stats.trim_mean(a, 2 / 6.0, axis=None)
    ...
    def test_empty(self, xp, scp, dtype, propotiontocut):
        a = xp.array([])
        return scp.stats.trim_mean(a, 2 / 6.0, propotiontocut)
    ...
    def test_axis(self, xp, scp, dtype, order, axis):
        ...
        return scp.stats.trim_mean(a, 2 / 6.0, axis=axis)
