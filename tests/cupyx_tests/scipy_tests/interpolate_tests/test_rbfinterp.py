...
class _TestRBFInterpolator:
    ...
    def test_extreme_domains(self, xp, scp, kernel):
        ...
        if kernel in _SCALE_INVARIANT:
            ...
            yitp2 = self.build(scp, x * scale + shift, y, kernel=kernel)(
                xitp * scale + shift
            )
        else:
            yitp1 = self.build(scp, x, y, epsilon=5.0, kernel=kernel)(xitp)
            yitp2 = self.build(
                scp, x * scale + shift, y, epsilon=5.0 / scale, kernel=kernel
            )(xitp * scale + shift)
    ...
    def test_smoothing_misfit(self, xp, scp, kernel):
        ...
        for smoothing in smoothing_range:
            ysmooth = self.build(
                scp, x, y, epsilon=1.0, smoothing=smoothing, kernel=kernel
            )(x)
    ...
    @pytest.mark.parametrize(
        "kernel", [kl for kl in _NAME_TO_MIN_DEGREE if _NAME_TO_MIN_DEGREE[kl] >= 1]
    )
...
class TestRBFInterpolatorNeighborsInf(TestRBFInterpolatorNeighborsNone):
    ...
    def build(self, scp, *args, **kwargs):
        return scp.interpolate.RBFInterpolator(*args, **kwargs, neighbors=cp.inf)
