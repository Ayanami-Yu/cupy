...
class TestNdBSpline:
    ...
    def test_2D_random(self, xp, scp):
        ...
        xi = xp.asarray([1.0, 1.0], dtype=xp.float64)
        r1 = spl(xi)

        xi = xp.c_[[1, 1.5, 2], [1.1, 1.6, 2.1]]
        return r1, spl(xi)
    ...
    def test_tx_neq_ty(self, xp, scp):
        ...
        bspl = scp.interpolate.NdBSpline((spl_x.t, spl_y.t), cc, (spl_x.k, spl_y.k))
    ...
    @pytest.mark.parametrize(
        "nu", [(1, 0, 0), (2, 0, 0), (2, 1, 0), (2, 1, 3), (2, 1, 4)]
    )
    @testing.numpy_cupy_allclose(scipy_name="scp")
    ...
    def test_3D_random(self, xp, scp):
        ...
        c = testing.shaped_random(
            (tx.size - k - 1, ty.size - k - 1, tz.size - k - 1), xp, xp.float64, 1.0
        )
        ...
        xi = xp.c_[[1, 1.5, 2], [1.1, 1.6, 2.1], [0.9, 1.4, 1.9]]
        return r1, spl(xi)
    ...
    def test_design_matrix_2(self, xp, scp, k):
        xvals = [(a, b) for a, b in itertools.product(xp.arange(6), xp.arange(7) + 1.5)]
        xvals = xp.asarray(xvals)

        t = (
            xp.array([0.0, 0.0, 0.0, 0.0, 2.0, 3.0, 5.0, 5.0, 5.0, 5.0]),
...
class TestMakeND:
    ...
    def _make(self, xp):
        if xp == cupy:
            ...
            return functools.partial(
                scipy.interpolate._ndbspline.make_ndbspl, solver=ssl.spsolve
            )
    ...
    def _get_sample_2d_data(self, xp):
        ...
        x = xp.array([0.5, 2.0, 3.0, 4.0, 5.5, 6.0])
        y = xp.array([0.5, 2.0, 3.0, 4.0, 5.5, 6.0])
        z = xp.array(
            [
                [1, 2, 1, 2, 1, 1],
                [1, 2, 1, 2, 1, 1],
