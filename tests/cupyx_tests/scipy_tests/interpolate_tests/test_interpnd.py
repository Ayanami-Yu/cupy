...
class TestLinearNDInterpolator:
    ...
    def test_smoketest_alternate(self, xp, scp):
        ...
        yi = scp.interpolate.LinearNDInterpolator((x[:, 0], x[:, 1]), y)(
            x_test[:, 0], x_test[:, 1]
        )
        return yi
    ...
    def test_square(self, xp, scp):
        ...
        values = xp.array([1.0, 2.0, -3.0, 5.0], dtype=xp.float64)

        xx, yy = xp.broadcast_arrays(
            xp.linspace(0, 1, 14)[:, None], xp.linspace(0, 1, 14)[None, :]
        )
        xx = xx.ravel()
        yy = yy.ravel()
    ...
    def test_square_rescale(self, xp, scp):
        ...
        points = xp.array([(0, 0), (0, 100), (10, 100), (10, 0)], dtype=xp.float64)
        values = xp.array([1.0, 2.0, -3.0, 5.0], dtype=xp.float64)

        xx, yy = xp.broadcast_arrays(
            xp.linspace(0, 10, 14)[:, None], xp.linspace(0, 100, 14)[None, :]
        )
        xx = xx.ravel()
        yy = yy.ravel()
        xi = xp.array([xx, yy]).T.copy()
        zi = scp.interpolate.LinearNDInterpolator(points, values, rescale=True)(xi)
        return zi
...
class TestEstimateGradients2DGlobal:
    ...
    def test_smoketest(self, func, xp, scp):
        x = xp.array(
            [(0, 0), (0, 2), (1, 0), (1, 2), (0.25, 0.75), (0.6, 0.8)], dtype=xp.float64
        )
...
def compute_random_points(tri_points, xp):
    ...
    s = testing.shaped_random((tri_points.shape[0],), np, np.float64, scale=1.0)
    t = testing.shaped_random((tri_points.shape[0],), np, np.float64, scale=1 - s)
    bary[:, 0] = xp.asarray(s)
    bary[:, 1] = xp.asarray(t)
...
class TestCloughTocher2DInterpolator:
    ...
    def test_tri_input_rescale(self, xp, scp):
        # Test at single points
        x = xp.array([(0, 0), (-5, -5), (-5, 5), (5, 5), (2.5, 3)], dtype=np.float64)
        y = xp.arange(x.shape[0], dtype=xp.float64)
        y = y - 3j * y
    ...
    def test_dense(self, func, rescale, xp, scp):
        ...
        grid = xp.r_[
            xp.array([(0, 0), (0, 1), (1, 0), (1, 1)], dtype=xp.float64),
            testing.shaped_random((30 * 30, 2), xp, cupy.float64, 1, 4321),
        ]
