...
class TestBarycentric:
    ...
    @testing.for_all_dtypes(no_bool=True, no_complex=True)
    @testing.numpy_cupy_allclose(scipy_name="scp", accept_error=OverflowError)
...
class TestCubicSpline:
    ...
    @pytest.mark.parametrize(
        "bc_type",
        ["not-a-knot", "clamped", "natural", ((1, 0), (1, 0)), ((1, 0), (2, 1.0))],
    )
...
class TestInterp1D:
    ...
    def setup_method(self):
        ...
        self.y210 = numpy.arange(20.0).reshape((2, 10))
        self.y102 = numpy.arange(20.0).reshape((10, 2))
        self.y225 = numpy.arange(20.0).reshape((2, 2, 5))
        self.y25 = numpy.arange(10.0).reshape((2, 5))
        self.y235 = numpy.arange(30.0).reshape((2, 3, 5))
        self.y325 = numpy.arange(30.0).reshape((3, 2, 5))

        # Edge updated test matrix 1
        # array([[ 30,   1,   2,   3,   4,   5,   6,   7,   8, -30],
        #        [ 30,  11,  12,  13,  14,  15,  16,  17,  18, -30]])
        self.y210_edge_updated = numpy.arange(20.0).reshape((2, 10))
        ...
        #       [ 16,  17],
        #       [-30, -30]])
        self.y102_edge_updated = numpy.arange(20.0).reshape((10, 2))
    ...
    def test_init(self):
        ...
        assert interp1d(x10, y10, fill_value=(1.0, 2.0)).fill_value == (1.0, 2.0)
    ...
    def test_assume_sorted_2D(self, xp, scp):
        ...
        interp10_y_2d_unsorted = scp.interpolate.interp1d(x10[::-1], y210[:, ::-1])
    ...
    def test_linear(self, xp, scp, kind):
        ...
        extrapolator = scp.interpolate.interp1d(
            x10, y10, kind=kind, fill_value="extrapolate"
        )
        xval = xp.asarray([-1.0, 0, 9, 11])
    ...
    @pytest.mark.parametrize(
        "dt_rc", ["float16", "float32", "float64", "complex64", "complex128"]
    )
    ...
    def test_slinear_dtypes(self, xp, scp, dt_n, dt_r, dt_rc, kind):
        ...
        xnew = x.astype(dt_n)
        return scp.interpolate.interp1d(x, y, kind=kind, bounds_error=False)(xnew)
    ...
    @testing.with_requires("scipy>=1.10")
    @pytest.mark.parametrize("kind", ["nearest", "nearest-up", "previous", "next"])
    @testing.numpy_cupy_allclose(scipy_name="scp")
    def test_nearest(self, xp, scp, kind):
        ...
        fe = scp.interpolate.interp1d(x10, y10, kind=kind, fill_value="extrapolate")
        xval = xp.asarray([2.4, 5.6, 6.0])
        xe = xp.asarray([-1.0, 0, 9, 11])
    ...
    def test_previous_2(self, xp, scp, kind):
        ...
        interpolator1D = scp.interpolate.interp1d(
            x10, y10, kind=kind, fill_value="extrapolate"
        )
        interpolator2D = scp.interpolate.interp1d(
            x10, y210, kind=kind, fill_value="extrapolate"
        )
        ...
        return (interpolator1D(xval), interpolator2D(xval), interpolator2DAxis0(xval2))
    ...
    def test_previous_3(self, xp, scp, kind):
        ...
        interpolator1D = scp.interpolate.interp1d(
            x, y, kind=kind, fill_value="extrapolate", assume_sorted=True
        )

        x1 = xp.asarray([2, 0, 1])  # x is not ascending
        y1 = xp.asarray([-1, 0, 1])
        interpolator1D_ns = scp.interpolate.interp1d(
            x1, y1, kind=kind, fill_value="extrapolate", assume_sorted=True
        )
    ...
    def test_bounds(self, kind, xp, scp):
        ...
        extrap10 = scp.interpolate.interp1d(
            x10, y10, fill_value=fill_value, bounds_error=False, kind=kind
        )
    ...
    def test_bounds_nan_fill(self, kind, xp, scp):
        ...
        c = scp.interpolate.interp1d(
            x, y, kind=kind, fill_value=cupy.nan, bounds_error=False
        )
        yi = c(x - 1)
        return yi
    ...
    def _check_fill_value(self, kind):
        ...
        y235, y325, y225, y25 = map(
            cupy.asarray, (self.y235, self.y325, self.y225, self.y25)
        )

        interp = interp1d(
            x10, y10, kind=kind, fill_value=(-100, 100), bounds_error=False
        )
        ...
        for y in (y235, y325, y225, y25):
            interp = interp1d(
                x5, y, kind=kind, axis=-1, fill_value=100, bounds_error=False
            )
            assert_array_almost_equal(interp(10), 100)
            assert_array_almost_equal(interp(-10), 100)
            ...
            interp = interp1d(
                x5, y, kind=kind, axis=-1, fill_value=(-100, 100), bounds_error=False
            )
            assert_array_almost_equal(interp(10), 100)
            assert_array_almost_equal(interp(-10), -100)
            if y.ndim == 3:
        ...
        for y in (y225, y325, y25):
            interp = interp1d(
                x5, y, kind=kind, axis=-1, fill_value=fill_value, bounds_error=False
            )
            result = [100, 200]
            if y.ndim == 3:
        ...
        interp = interp1d(
            x5, y235, kind=kind, axis=-1, fill_value=fill_value, bounds_error=False
        )
        assert_array_almost_equal(interp(10), 100)
        assert_array_almost_equal(interp(-10), [[-100, -200, -300]] * 2)
        assert_array_almost_equal(
        ...
        for ii in range(2):
            ...
            interp = interp1d(
                x5, y225, kind=kind, axis=-1, fill_value=fill_value, bounds_error=False
            )
        ...
        for ii in range(2):
            if ii == 1:
                fill_value = (cupy.array(fill_value[0]), cupy.array(fill_value[1]))
            interp = interp1d(
                x5, y225, kind=kind, axis=-1, fill_value=fill_value, bounds_error=False
            )
            assert_array_almost_equal(interp(10), [[100, 200], [1000, 2000]])
    ...
    def _nd_check_interp(self, kind="linear"):
        ...
        assert_array_almost_equal(
            interp10(cupy.array([[3.0, 5.0], [2.0, 7.0]])),
            cupy.array([[3.0, 5.0], [2.0, 7.0]]),
        )
        ...
        assert_array_almost_equal(interp210(1.0), cupy.array([1.0, 11.0]))
        assert_array_almost_equal(
            interp210(cupy.array([1.0, 2.0])), cupy.array([[1.0, 2.0], [11.0, 12.0]])
        )

        interp102 = interp1d(x10, y102, axis=0, kind=kind)
        assert_array_almost_equal(interp102(1.0), cupy.array([2.0, 3.0]))
        assert_array_almost_equal(
            interp102(cupy.array([1.0, 3.0])), cupy.array([[2.0, 3.0], [6.0, 7.0]])
        )

        # Both at the same time!
        x_new = cupy.array([[3.0, 5.0], [2.0, 7.0]])
        assert_array_almost_equal(
            interp210(x_new),
            cupy.array([[[3.0, 5.0], [2.0, 7.0]], [[13.0, 15.0], [12.0, 17.0]]]),
        )
        assert_array_almost_equal(
            interp102(x_new),
            cupy.array([[[6.0, 7.0], [10.0, 11.0]], [[4.0, 5.0], [14.0, 15.0]]]),
        )
    ...
    def _nd_check_shape(self, kind="linear"):
        ...
        for n, s in enumerate(a):
            ...
            x2 = cupy.arange(2 * 3 * 1).reshape((2, 3, 1)) / 12.0
            b = list(a)
            b[n : n + 1] = [2, 3, 1]
