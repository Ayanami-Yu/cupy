...
class TestRegularGridInterpolator:
    ...
    def test_length_one_axis(self):
        ...
        assert_allclose(
            interp(cp.array([[1, 1], [1, 5], [1, 10]])), [2, 6, 11], atol=1e-14
        )

        # check off-grid interpolation is indeed linear
        assert_allclose(
            interp(cp.array([[1, 1.4], [1, 5.3], [1, 10]])), [2.4, 6.3, 11], atol=1e-14
        )
    ...
    def test_invalid_points_order(self):
        ...
        x = cp.array([0.5, 2.0, 0.0, 4.0, 5.5])  # not ascending or descending
    ...
    def test_fill_value(self, method):
        interp = RegularGridInterpolator(
            [cp.arange(6)], cp.ones(6), method=method, bounds_error=False
        )
    ...
    def test_derivatives(self):
        ...
        assert_allclose(interp(sample, nu=(0, 1, 1, 0)), [0, 0, 0], atol=1e-12)
