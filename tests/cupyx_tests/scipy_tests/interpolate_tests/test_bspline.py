...
class TestBSpline:
    ...
    def test_bspline_endpoints(self, xp, scp):
        ...
        return (
            ...
            b(xp.asarray([tm + 1e-10, tp - 1e-10]), extrapolate=self.extrapolate),
        )
    ...
    def test_bspline_extrap(self, xp, scp):
        ...
        return (
            b(xx[mask], extrapolate=self.extrapolate),
    ...
    def test_axis(self, xp, scp):
        ...
        BSpline = scp.interpolate.BSpline
        for b1 in [
            BSpline(t, c, k, axis=self.axis).derivative(),
    ...
    @testing.numpy_cupy_allclose(scipy_name="scp", rtol=3e-5, atol=3e-5)
    @testing.with_requires("scipy>=1.8.0")
    def test_design_matrix_same_as_BSpline_call(self, xp, scp):
        ...
        for k in range(0, 5):
            ...
            ret.append(
                scp.interpolate.BSpline.design_matrix(x, t, k, **kwargs).todense()
            )
