...
class TestResample:
    ...
    def test_rfft(self, N, num, window, xp, scp):
        ...
        results.append(
            scp.signal.resample(y_complex, num, axis=1, window=window).real.copy()
        )
        return results
    ...
    def test_input_domain(self, xp, scp):
        ...
        num = 256
        return (
            scp.signal.resample(fsig, num, domain="freq"),
...
class TestDecimate:
    ...
    def _test_phaseshift(self, xp, scp, method, zero_phase):
        ...
        for rate_to in rates_to:
            ...
            d_tos = xp.exp(
                1j * 2 * xp.pi * freqs[:, xp.newaxis] * t_to
            ...
            if method == "fir":
                ...
                system = scp.signal.dlti(
                    scp.signal.firwin(n + 1, 1.0 / q, window="hamming"), 1.0
                )
            elif method == "iir":
                n = 8
                wc = 0.8 * xp.pi / q
