...
class TestUniqueRoots:
    ...
    def test_real_repeat(self, xp, scp, rtype):
        ...
        unique, multiplicity = scp.signal.unique_roots(p, tol=1e-1, rtype=rtype)
        return unique, multiplicity
    ...
    def test_complex_repeat(self, xp, scp, rtype):
        ...
        unique, multiplicity = scp.signal.unique_roots(p, tol=1e-1, rtype="min")
        return unique, multiplicity
...
class TestPartialFractionExpansion:
    ...
    def test_compute_factors(self):
        ...
        testing.assert_allclose(factors[2], [1, -7, 19, -25, 16, -4], atol=1e-15)
        testing.assert_allclose(poly, [1, -10, 40, -82, 91, -52, 12], atol=1e-15)

        factors, poly = _compute_factors([1, 2, 3], [3, 2, 1], include_powers=True)
        assert len(factors) == 6
        testing.assert_allclose(factors[0], [1, -9, 31, -51, 40, -12], atol=1e-15)
        testing.assert_allclose(factors[1], [1, -8, 23, -28, 12], atol=1e-15)
        testing.assert_allclose(factors[2], [1, -7, 16, -12], atol=1e-15)
        testing.assert_allclose(factors[3], [1, -8, 24, -34, 23, -6], atol=1e-15)
        testing.assert_allclose(factors[4], [1, -6, 12, -10, 3], atol=1e-15)
        testing.assert_allclose(factors[5], [1, -7, 19, -25, 16, -4], atol=1e-15)
    ...
    @pytest.mark.parametrize("ba", [([0, 0], [1, 6, 8]), (0, 1)])
    @testing.numpy_cupy_allclose(scipy_name="scp")
