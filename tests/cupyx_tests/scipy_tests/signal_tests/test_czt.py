...
@pytest.mark.parametrize(
    ...
    (
        [0, 0, 1],
        [0, 0, 1, 0, 0],
        cupy.concatenate((cupy.array([0, 0, 1]), cupy.zeros(100))),
    ),
)
...
def test_czt_math(impulse, m, w, a):
    ...
    assert_allclose(
        signal.czt(impulse[1:], m=m, w=w, a=a),
        signal.czt_points(m=m, w=w, a=a) ** -1,
        rtol=1e-10,
    )

    # z-transform of a 2-delayed impulse is z**-2
    assert_allclose(
        signal.czt(impulse, m=m, w=w, a=a),
        signal.czt_points(m=m, w=w, a=a) ** -2,
        rtol=1e-10,
    )
