...
class TestWelch:
    ...
    def test_detrend_external(self, xp, scp):
        ...
        f, p = scp.signal.welch(
            x, nperseg=10, detrend=lambda seg: scp.signal.detrend(seg, type="l")
        )
        return f, p
...
class TestCSD:
    ...
    def test_unk_scaling(self, mod):
        ...
        with pytest.raises(ValueError):
            scp.signal.csd(
                xp.zeros(4, np.complex128),
                xp.ones(4, np.complex128),
                scaling="foo",
                nperseg=4,
            )
    ...
    def test_detrend_external(self, xp, scp):
        x = xp.arange(10, dtype=xp.float64) + 0.04
        f, p = scp.signal.csd(
            x, x, nperseg=10, detrend=lambda seg: scp.signal.detrend(seg, type="l")
        )
        return f, p
    ...
    @pytest.mark.skipif(
        cupy.cuda.runtime.runtimeGetVersion() < 12000, reason="It fails on CUDA 11.x"
    )
    @testing.numpy_cupy_allclose(scipy_name="scp", rtol=1e-5, atol=1e-5)
    def test_copied_data(self, xp, scp):
        ...
        _, p_same1 = scp.signal.csd(
            x, x, nperseg=8, average="mean", return_onesided=False
        )
        _, p_copied1 = scp.signal.csd(
            x, y, nperseg=8, average="mean", return_onesided=False
        )

        _, p_same2 = scp.signal.csd(
            x, x, nperseg=8, average="median", return_onesided=False
        )
        _, p_copied2 = scp.signal.csd(
...
class TestSTFT:
    ...
    def test_roundtrip_scaling(self, xp, scp):
        ...
        Zp = scp.signal.stft(x, return_onesided=False, boundary="even", scaling="psd")[
            2
        ]
        results.append(Zp)
        ...
        Zp0 = scp.signal.stft(x, return_onesided=True, boundary="even", scaling="psd")[
            2
        ]
        results.append(Zp0)
