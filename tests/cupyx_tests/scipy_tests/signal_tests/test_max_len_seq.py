...
class TestMLS:
    ...
    def test_mls_output(self, nbits, state, taps, xp, scp):
        ...
        tester = xp.real(scp.fft.ifft(scp.fft.fft(m) * xp.conj(scp.fft.fft(m))))
        out_len = 2**nbits - 1
        ...
        for n in (1, 2 ** (nbits - 1)):
            m1, s1 = scp.signal.max_len_seq(nbits, state=state, taps=taps, length=n)
            m2, s2 = scp.signal.max_len_seq(nbits, state=s1, taps=taps, length=1)
