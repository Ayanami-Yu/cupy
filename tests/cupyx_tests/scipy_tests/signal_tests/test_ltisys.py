...
class TestPlacePoles:
    ...
    @pytest.mark.xfail(
        platform.machine() == "aarch64", reason="passes locally, fails on aarch64"
    )
    @testing.numpy_cupy_allclose(scipy_name="scp")
    ...
    def test_complex(self, xp, scp):
        ...
        # editions p 184/185
        A = xp.array([[0, 7, 0, 0], [0, 0, 0, 7 / 3.0], [0, 0, 0, 0], [0, 0, 0, 0]])
    ...
    def test_errors(self):
        ...
        A = cupy.array([0, 7, 0, 0, 0, 0, 0, 7 / 3.0, 0, 0, 0, 0, 0, 0, 0, 0]).reshape(
            4, 4
        )
        ...
        with assert_raises(ValueError):
            signal.place_poles(A, B, cupy.array([-2.1, -2.2, -2.3, -2.4]), method="foo")
