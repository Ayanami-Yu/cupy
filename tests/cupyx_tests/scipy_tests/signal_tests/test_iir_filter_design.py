...
class TestIIRFilter:
    ...
    @pytest.mark.parametrize(
        "ftype",
        ["butter", pytest.param("bessel", marks=nimpl), "cheby1", "cheby2", "ellip"],
    )
...
class TestButter:
    ...
    @pytest.mark.parametrize(
        "outp", ["zpk", "sos", pytest.param("ba", marks=prec_loss)]
    )
    @testing.numpy_cupy_allclose(scipy_name="scp")
    @testing.with_requires("scipy>=1.8")
...
class TestIIRDesign:
    ...
    def test_exceptions(self):
        ...
        with pytest.raises(ValueError, match="the same shape"):
            iirdesign(
                cupy.array([[0.3, 0.6], [0.3, 0.6]]),
                cupy.array([[0.4, 0.5], [0.4, 0.5]]),
                1,
                40,
            )
