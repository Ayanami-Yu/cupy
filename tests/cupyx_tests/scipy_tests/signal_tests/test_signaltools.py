...
class TestConvolveCorrelate:
    ...
    tols = {np.float32: 1e-5, np.complex64: 1e-5, np.float16: 1e-3, "default": 1e-10}

    @testing.for_all_dtypes()
    @testing.numpy_cupy_allclose(
        atol=tols, rtol=tols, scipy_name="scp", accept_error=ValueError
    )
...
class TestFFTConvolve:
    ...
    tols = {np.float32: 1e-3, np.complex64: 1e-3, np.float16: 1e-3, "default": 1e-8}
...
class TestConvolveCorrelate2D:
    def _filter(self, func, dtype, xp, scp):
        ...
        return getattr(scp.signal, func)(
            in1, in2, self.mode, self.boundary, self.fillvalue
        )

    tols = {np.float32: 5e-4, np.complex64: 5e-4, np.float16: 1e-3, "default": 1e-10}
...
class TestOrderFilter:
    ...
    @testing.numpy_cupy_allclose(
        atol=1e-8, rtol=1e-8, scipy_name="scp", accept_error=ValueError
    )  # for even kernels
...
class TestLFilter:
    ...
    def test_fir_iir_order(
        self, size, fir_order, iir_order, in_dtype, const_dtype, xp, scp
    ):
        out_dtype = xp.result_type(in_dtype, const_dtype)
        if xp.dtype(out_dtype).kind in {"i", "u"}:
            pytest.skip()
        if runtime.is_hip and driver.get_build_version() < 5_00_00000 and iir_order > 0:
    ...
    def test_fir_iir_order_ndim(
        self, size, fir_order, iir_order, axis, in_dtype, const_dtype, xp, scp
    ):
        out_dtype = xp.result_type(in_dtype, const_dtype)
        if xp.dtype(out_dtype).kind in {"i", "u"}:
            pytest.skip()
    ...
    def test_fir_iir_zi_ndim(
        ...
        if xp is not cupy:
            ...
            zi_m_shape = zi.shape
            zi = zi.reshape(
                int(np.prod(zi_m_shape[:-1])), fir_order + iir_order - 1
            ).copy()
            zi = xp.concatenate(
                [
                    scp.signal.lfiltic(
...
class TestSosFilt:
        ...
        if xp is not cupy:
            ...
            for s in range(sections):
                ...
                section_zi = xp.concatenate(
                    [
                        scp.signal.lfiltic(b, a, z[2:][::-1], z[:2][::-1])
                        for z in section_zi
                    ]
                )
    ...
    @pytest.mark.parametrize("zeros", [(4,), (5,), (4, 5)])
    @testing.numpy_cupy_array_almost_equal(scipy_name="scp", decimal=5)
...
class TestFiltFilt:
    ...
    @testing.numpy_cupy_allclose(
        scipy_name="scp", rtol=1e-3, atol=1e-2, type_check=False, accept_error=True
    )
...
class TestSosFiltFilt:
    ...
    @testing.for_all_dtypes_combination(no_float16=True, no_bool=True, names=("dtype",))
    @testing.numpy_cupy_allclose(
        scipy_name="scp", rtol=5e-3, type_check=False, accept_error=True
    )
