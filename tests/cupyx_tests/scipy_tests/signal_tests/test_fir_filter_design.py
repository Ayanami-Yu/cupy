...
class TestFirwin:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ((53, 0.5), dict(pass_zero=False)),  # stop from 0 to f --> high-pass
...
class TestFirwin2:
    ...
    def test_invalid_args(self):
        ...
        with assert_raises(
            ...
            signal.firwin2(
                50, [0.0, 0.5 - eps * 0.5, 0.5, 0.5, 1.0], [1.0, 1.0, 1.0, 0.0, 0.0]
            )
    ...
    def test_tuple(self, xp, scp):
        taps1 = scp.signal.firwin2(
            150, xp.asarray((0.0, 0.5, 0.5, 1.0)), xp.asarray((1.0, 1.0, 0.0, 0.0))
        )
        taps2 = scp.signal.firwin2(
            150, xp.asarray([0.0, 0.5, 0.5, 1.0]), xp.asarray([1.0, 1.0, 0.0, 0.0])
        )
...
class TestFirls:
    ...
    def test_compare(self, xp, scp):
        ...
        taps = scp.signal.firls(9, [0, 0.5, 0.55, 1], [1, 1, 0, 0], weight=[1, 2])
        return taps

    @testing.numpy_cupy_allclose(scipy_name="scp")
    def test_compare_2(self, xp, scp):
        # compare to MATLAB output
        taps = scp.signal.firls(11, [0, 0.5, 0.5, 1], [1, 1, 0, 0], weight=[1, 2])
        return taps
