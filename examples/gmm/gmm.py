...
def estimate_log_prob(X, inv_cov, means):
    ...
    log_prob = (
        ...
        - 2 * xp.dot(X, (means * precisions).T)
        + xp.dot(X**2, precisions.T)
    )
...
def draw(X, pred, means, covariances, output):
    ...
    plt.scatter(
        means[:, 0], means[:, 1], s=120, marker="s", facecolors="y", edgecolors="k"
    )
...
def run(gpuid, num, dim, max_iter, tol, output):
    ...
    print("Running CPU...")
    with timer(" CPU "):
        y_test_pred, means, cov = calc_acc(
            X_train, y_train, X_test, y_test, max_iter, tol, means, covariances
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num", "-n", default=500000, type=int, help="number of train data"
    )
    parser.add_argument(
        "--dim", "-d", default=2, type=int, help="dimension of each data"
    )
    parser.add_argument(
        "--max-iter", "-m", default=30, type=int, help="number of iterations"
    )
    parser.add_argument(
        "--tol",
        "-t",
