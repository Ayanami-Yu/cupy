...
from cupy_backends.cuda.libs.cusparselt import (
    Handle,
    MatDescriptor,
    MatmulDescriptor,
    MatmulAlgSelection,
...
cusparselt.structuredDescriptorInit(
    handle,
    matA,
    A.shape[0],
    A.shape[1],
...
cusparselt.denseDescriptorInit(
    handle, matB, B.shape[0], B.shape[1], B.shape[1], alignment, cuda_dtype, order
)
cusparselt.denseDescriptorInit(
    handle, matC, C.shape[0], C.shape[1], C.shape[1], alignment, cuda_dtype, order
)
...
cusparselt.matmulDescriptorInit(
    handle, matmul, opA, opB, matA, matB, matC, matC, compute_type
)
cusparselt.matmulAlgSelectionInit(
    handle, alg_sel, matmul, cusparselt.CUSPARSELT_MATMUL_ALG_DEFAULT
)
...
cusparselt.spMMAPrune(
    handle, matmul, A.data.ptr, A.data.ptr, cusparselt.CUSPARSELT_PRUNE_SPMMA_TILE
)
...
cusparselt.spMMACompress(
    handle, plan, A.data.ptr, A_compressed.data.ptr, A_compressedBuffer.data.ptr
)
