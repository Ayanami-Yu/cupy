...
sum_kernel = cupy.ReductionKernel(
    "T x, S mask", "T out", "mask ? x : 0", "a + b", "out = a", "0", "sum_kernel"
)
...
def fit_custom(X, n_clusters, max_iter):
    ...
    for _ in range(max_iter):
        distances = var_kernel(
            X[:, None, 0], X[:, None, 1], centers[None, :, 1], centers[None, :, 0]
        )
...
def draw(X, n_clusters, centers, pred, output):
    ...
    plt.scatter(
        centers[:, 0], centers[:, 1], s=120, marker="s", facecolors="y", edgecolors="k"
    )
    plt.savefig(output)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-clusters", "-n", default=2, type=int, help="number of clusters"
    )
    parser.add_argument("--num", default=5000000, type=int, help="number of samples")
    parser.add_argument(
        "--max-iter", "-m", default=10, type=int, help="number of iterations"
    )
    parser.add_argument(
        "--use-custom-kernel",
        action="store_true",
    ...
    parser.add_argument(
        "--output-image", "-o", default=None, type=str, help="output image file name"
    )
