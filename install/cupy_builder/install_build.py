    ...
    try:
        ...
        try:
            ...
            cupy_cub_include = os.path.join(
                cupy_builder.get_context().source_root, "third_party/cccl"
            )
            a = subprocess.run(
            ...
            if a.returncode == 0:
                ...
                raise RuntimeError(
                    "Cannot determine CUB version from git tag" "\n{}".format(e)
                )
        except Exception as e:
...
def check_jitify_version(compiler, settings):
    ...
    try:
        cupy_jitify_include = os.path.join(
            cupy_builder.get_context().source_root, "third_party/jitify"
        )
...
def conda_update_dirs(include_dirs, library_dirs):
    ...
    if int(os.environ.get("CONDA_BUILD_CROSS_COMPILATION", 0)) == 1:
        ...
        if os.environ.get("CONDA_OVERRIDE_CUDA", "0").startswith("12"):
            include_dirs.insert(
                0, f'{os.environ["BUILD_PREFIX"]}/targets/x86_64-linux/include'
            )
            library_dirs.insert(
    ...
    if os.environ.get("CONDA_OVERRIDE_CUDA", "0").startswith("12"):
        if PLATFORM_LINUX:
            include_dirs.append(
                ...
                f"{conda_get_target_name()}/include"
            )  # for crt headers
            ...
            # for optional dependencies
            include_dirs.append(
                f'{os.environ["LIBRARY_INC"]}'
            )  # $PREFIX/Library/include
            library_dirs.append(f'{os.environ["LIBRARY_LIB"]}')  # $PREFIX/Library/lib
