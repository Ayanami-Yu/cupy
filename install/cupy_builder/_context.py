...
def _get_env_bool(name: str, env: Mapping[str, str], *, default: bool = False) -> bool:
...
class Context:
    ...
    def calculate_cupy_cache_key(self) -> None:
        ...
        pattern = os.path.join(self.source_root, "cupy", "_core", "include", "**")
        cache_key, count = self._calculate_checksum(pattern)
        print(f"CUPY_CACHE_KEY ({count} files " f"matching {pattern}): {cache_key}")
        self.cupy_cache_key = cache_key

    def calculate_dev_cache_key(self) -> None:
        print("Generating configure cache key...")
        pattern = os.path.join(self.source_root, "install", "cupy_builder", "**")
        cache_key, count = self._calculate_checksum(pattern)
        print(
            f"Configure cache key ({count} files " f"matching {pattern}): {cache_key}"
        )
