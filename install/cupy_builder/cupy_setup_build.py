...
def preconfigure_modules(ctx: Context, MODULES, compiler, settings):
    ...
    summary = [
        ...
        "  hipcc command      : {}".format(hipcc_path if hipcc_path else "(not found)"),
        "",
    ...
    for module in MODULES:
        ...
        elif module["name"] in ("thrust", "cub", "random") and (
            nvcc_path is None and hipcc_path is None
        ):
            installed = True
...
def _find_static_library(name: str) -> str:
    if PLATFORM_LINUX:
        ...
        if int(
            os.environ.get("CONDA_BUILD_CROSS_COMPILATION", 0)
        ) == 1 and os.environ.get("CONDA_OVERRIDE_CUDA", "0").startswith("11"):
            # CUDA 11 on conda-forge has an ad hoc layout to support cross
            # compiling
            libdirs = ["lib"]
            cuda_path = (
                f"{build.get_cuda_path()}/targets/" f"{build.conda_get_target_name()}/"
            )
        else:
            libdirs = ["lib64", "lib"]
...
def make_extensions(ctx: Context, compiler, use_cython):
    ...
    if ctx.dev_configure_cache and os.path.exists(CACHE_FILE):
        ...
        if (
            ctx.dev_configure_cache_key == prev_ctx.dev_configure_cache_key
            and ctx.cupy_cache_key == prev_ctx.cupy_cache_key
        ):
            print("***************************************************")
            print(
                "*** NOTICE: Reusing build configuration from previous "
    ...
    try:
        ...
        if int(os.environ.get("CONDA_BUILD_CROSS_COMPILATION", 0)) == 1:
            os.symlink(
                f'{os.environ["BUILD_PREFIX"]}/x86_64-conda-linux-gnu/'
                "bin/x86_64-conda-linux-gnu-ld",
                f'{os.environ["BUILD_PREFIX"]}/bin/ld',
            )
        if PLATFORM_LINUX and (
            int(os.environ.get("CONDA_BUILD_CROSS_COMPILATION", 0)) == 1
            or os.environ.get("CONDA_OVERRIDE_CUDA", "0").startswith("12")
        ):
        ...
        if no_cuda:
            ...
            if not (set(required_modules) <= set(available_modules)):
                raise Exception(
                    "Your CUDA environment is invalid. " "Please check above error log."
                )
    finally:
