...
def compile_device_code(
    ctx: Context, ext: setuptools.Extension
) -> tuple[list[str], list[str]]:
...
class custom_build_ext(setuptools.command.build_ext.build_ext):
    ...
    def build_extensions(self) -> None:
        ...
        for ext in self.extensions:
            ...
            sources_cpp = [
                "{}.cpp".format(os.path.splitext(src)[0]) for src in sources_pyx
            ]
    ...
    def build_extension(self, ext: setuptools.Extension) -> None:
        ...
        if ctx.setup_command == "editable_wheel" and not self.force:
            ...
            if os.path.exists(ext_inplace) and max(
                _get_timestamp(f) for f in (ext.sources + ext.depends)
