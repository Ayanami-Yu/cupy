    ...
    for key, key_schema in schema.items():
        if key == "os":
            for value, value_schema in key_schema.items():
                ...
                if system is None:
                    raise ValueError(
                        f"system is missing " f"while parsing schema os:{value}"
                    )
                if system not in schema["system"].keys():
                    raise ValueError(
                        f"unknown system: {system} " f"while parsing schema os:{value}"
                    )
...
def validate_matrixes(schema: SchemaType, matrixes: list[Matrix]) -> None:
    ...
    for matrix in matrixes:
        ...
        if matrix.cuda is None and matrix.rocm is None:
            errors.append(f"{matrix.project}: Either cuda nor rocm must be non-null")

        if matrix.cuda is not None and matrix.rocm is not None:
            errors.append(f"{matrix.project}: cuda and rocm are mutually exclusive")
...
def main(argv: list[str]) -> int:
    ...
    for matrix in matrixes:
        ...
        if matrix.system == "linux":
            ...
            output[f"linux/tests/{matrix.target}.sh"] = gen.generate_script()
