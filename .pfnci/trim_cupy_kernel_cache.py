...
def main():
    ...
    for atime, size, _ in records:
        if (
            (expiry_ts is not None and atime < expiry_ts)
            or (options.max_files is not None and options.max_files <= keep_count)
            or (options.max_size is not None and options.max_size < (keep_size + size))
        ):
            break
        keep_count += 1
        keep_size += size
