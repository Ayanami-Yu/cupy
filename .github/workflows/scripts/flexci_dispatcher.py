...
def parse_args(argv: Any) -> Any:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["issue_comment", "push", "pull_request", "pull_request_target"],
...
def main(argv: Any) -> int:
    ...
    if len(projects_dispatch) == 0:
        ...
        success = _forward_to_flexci(
            event_name, payload, webhook_secret, projects_dispatch, options.flexci_uri
        )
        if not success:
