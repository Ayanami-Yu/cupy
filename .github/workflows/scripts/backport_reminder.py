...
def check_tbp_issue(issue, repo, bp_issues, grace_dt, interactive, verbose):
    ...
    try:
        ...
        if not found_backport and (issue.updated_at < grace_dt):
            ...
            if interactive:
                print(
                    "PR #{} (@{}): {} (last update: {})".format(
                        issue.number, mention_to, issue.title, issue.updated_at
                    )
                )
                return
            msg = (
                "@{} This pull-request is marked as `to-be-backported`, "
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Pull requests updated within the " "specified days will be checked",
    )
    parser.add_argument(
        "--inactive-days",
        type=int,
    ...
    print(
        f'"to-be-backported" pull-requests updated after {after_dt} will be '
