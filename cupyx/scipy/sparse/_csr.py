...
class csr_matrix(_compressed._compressed_sparse_matrix):
    ...
    def __mul__(self, other):
        ...
        if cupy.isscalar(other):
            ...
            if other.ndim == 0:
                ...
                if (
                    cusparse.check_availability("csrmvEx")
                    and self.nnz > 0
    ...
    def __truediv__(self, other):
        ...
        if _util.isscalarlike(other):
            ...
            check_shape_for_pointwise_op(
                self.shape, other.shape, allow_broadcasting=False
            )
...
def _cupy_csr_diagonal():
    return cupy.ElementwiseKernel(
        "int32 k, int32 rows, int32 cols, " "raw T data, raw I indptr, raw I indices",
        "T y",
        """
        int row = i;
        int col = i;
