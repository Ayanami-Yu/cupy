...
SYMIIR2_MODULE = cupy.RawModule(
    code=SYMIIR2_KERNEL,
    options=("-std=c++11",),
    name_expressions=[f"compute_symiirorder2_bwd_sc<{t}>" for t in ["float", "double"]]
    + [f"compute_symiirorder2_fwd_sc<{t}>" for t in ["float", "double"]],
)
...
def _find_initial_cond(all_valid, cum_poly, n, off=0, axis=-1):
    ...
    if indices.size > 0:
        zi = cupy.where(
            ...
            axis_slice(cum_poly, indices[0] - 1 - off, indices[0] - off, axis=axis),
        )
    return zi
...
def _symiirorder1_nd(input, c0, z1, precision=-1.0, axis=-1):
    ...
    y1, _ = apply_iir_sos(
        axis_slice(input, 1), coef, zi=all_zi, dtype=input.dtype, apply_fir=False
    )
    y1 = cupy.c_[zi, y1]
...
def _symiirorder2_nd(input, r, omega, precision=-1.0, axis=-1):
    ...
    for i in range(0, input.shape[-1] + 2, block_sz):
        compute_symiirorder2_fwd_sc(
            (1,),
            (block_sz + 1,),
            (input.shape[-1] + 2, i, cs, r, omega, precision, all_valid, diff),
        )
        ...
        if cupy.isnan(y0):
            ...
            y0 = _find_initial_cond(
                all_valid[:-1][: input_slice.shape[-1]], cum_poly_y0, input.shape[-1], i
            )

        if cupy.isnan(y1):
            cum_poly_y1 = (
                cupy.cumsum(diff_y1 * input_slice, axis=-1)
                + starting_diff[0] * axis_slice(input, 1, 2)
                + starting_diff[1] * axis_slice(input, 0, 1)
            )
            y1 = _find_initial_cond(
                all_valid[1:][: input_slice.shape[-1]], cum_poly_y1, input.shape[-1], i
            )
    ...
    for i in range(0, input.shape[-1] + 1, block_sz):
        compute_symiirorder2_bwd_sc(
            ...
            (
                ...
                1,
                cs,
                cupy.asarray(rsq, cs.dtype),
                cupy.asarray(omega, cs.dtype),
                precision,
                all_valid,
                diff,
            ),
