...
def _direct_correlate(
    ...
    if in1.ndim != 1 and (
        in1.dtype.kind == "b" or (in1.dtype.kind == "f" and in1.dtype.itemsize < 4)
    ):
...
def _freq_domain_conv(in1, in2, axes, shape, calc_fast_len=False):
    ...
    fshape = (
        [fft.next_fast_len(shape[a], real) for a in axes] if calc_fast_len else shape
    )
