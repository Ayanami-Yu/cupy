...
def make_first_tri(tri_arr, opp_arr, tri_info_arr, tri, inf_idx):
    ...
    ker_make_first_tri((1,), (1,), (tri_arr, opp_arr, tri_info_arr, tri, inf_idx))
...
def init_point_location_fast(
    ...
    ker_init_point_loc_fast = DELAUNAY_MODULE.get_function("kerInitPointLocationFast")
    ker_init_point_loc_fast(
        (N_BLOCKS,),
        (BLOCK_SZ,),
        (
            vert_tri,
...
def init_point_location_exact(
    ...
    ker_init_point_loc_fast = DELAUNAY_MODULE.get_function("kerInitPointLocationExact")
    ker_init_point_loc_fast(
        (PRED_N_BLOCKS,),
        (PRED_BLOCK_SZ,),
        (
...
def shift(shift_idx, in_arr, out_arr, type_str):
    ...
    ker_shift(
        (N_BLOCKS,), (BLOCK_SZ,), (shift_idx, int(shift_idx.shape[0]), in_arr, out_arr)
    )
...
def shift_tri_idx(idx, shift_idx):
    ...
    ker_shift((N_BLOCKS,), (BLOCK_SZ,), (idx, int(idx.shape[0]), shift_idx))


def split_points_fast(
    vert_tri,
    tri_to_vert,
    tri,
    ins_tri_map,
    ...
    ker_split_points_fast(
        (N_BLOCKS,),
        (BLOCK_SZ,),
        (
            vert_tri,
            int(vert_tri.shape[0]),
            tri_to_vert,
            tri,
            ins_tri_map,
            exact_check,
...
def split_points_exact(
    ...
    ker_split_points_exact = DELAUNAY_MODULE.get_function("kerSplitPointsExactSoS")
...
def split_tri(
    split_tri, tri, tri_opp, tri_info, ins_tri_map, tri_to_vert, tri_num, ins_tri_num
):
...
def is_tri_active(tri_info):
    ...
    ker_is_tri_active((N_BLOCKS,), (BLOCK_SZ,), (tri_info, out, tri_info.shape[0]))
    return out


def mark_special_tris(tri_info, tri_opp):
    ker_mark_special_tris = DELAUNAY_MODULE.get_function("kerMarkSpecialTris")
    ker_mark_special_tris(
        (N_BLOCKS,), (BLOCK_SZ,), (tri_info, tri_opp, tri_info.shape[0])
    )


def check_delaunay_fast(
    act_tri, tri, tri_opp, tri_info, tri_vote, act_tri_num, inf_idx, points, pred_consts
):
    ker_check_delaunay_fast = DELAUNAY_MODULE.get_function("kerCheckDelaunayFast")
    ker_check_delaunay_fast(
        (N_BLOCKS,),
        (BLOCK_SZ,),
        (
            act_tri,
...
def check_delaunay_exact_fast(
    ...
    ker_check_delaunay_e_f = DELAUNAY_MODULE.get_function("kerCheckDelaunayExact_Fast")
    ker_check_delaunay_e_f(
        (N_BLOCKS,),
        (BLOCK_SZ,),
        (
            act_tri,
...
def check_delaunay_exact_exact(
    tri,
    tri_opp,
    tri_vote,
    exact_check_vi,
    ...
    ker_check_delaunay_e_e = DELAUNAY_MODULE.get_function("kerCheckDelaunayExact_Exact")

    ker_check_delaunay_e_e(
        (PRED_N_BLOCKS,),
        (PRED_BLOCK_SZ,),
        (
            tri,
...
def mark_rejected_flips(act_tri, tri_opp, tri_vote, tri_info, flip_to_tri, act_tri_num):
    ker_mark_rejected_flips = DELAUNAY_MODULE.get_function("kerMarkRejectedFlips")
    ker_mark_rejected_flips(
        (N_BLOCKS,),
        (BLOCK_SZ,),
...
def update_opp(flip_vec, tri_opp, tri_msg, flip_to_tri, org_flip_num, flip_num):
...
def update_flip_trace(flip_arr, tri_to_flip, org_flip_num, flip_num):
    ...
    ker_update_flip_trace(
        (N_BLOCKS,), (BLOCK_SZ,), (flip_arr, tri_to_flip, org_flip_num, flip_num)
    )


def relocate_points_fast(
    vert_tri,
    tri_to_flip,
    flip_arr,
    exact_check,
    ...
    ker_relocate_points_fast = DELAUNAY_MODULE.get_function("kerRelocatePointsFast")
    ker_relocate_points_fast(
        (N_BLOCKS,),
        (BLOCK_SZ,),
        (
            vert_tri,
...
def relocate_points_exact(
    ...
    ker_relocate_points_exact = DELAUNAY_MODULE.get_function("kerRelocatePointsExact")
    ker_relocate_points_exact(
        (N_BLOCKS,),
        (BLOCK_SZ,),
        (
            vert_tri,
...
def mark_inf_tri(tri, tri_info, opp, inf_idx):
    ...
    ker_mark_inf_tri(
        (N_BLOCKS,), (BLOCK_SZ,), (tri, tri.shape[0], tri_info, opp, inf_idx)
    )
...
def collect_free_slots(tri_info, prefix, free_arr, new_tri_num):
    ...
    ker_collect_free(
        (N_BLOCKS,), (BLOCK_SZ,), (tri_info, prefix, free_arr, new_tri_num)
    )
...
def make_key_from_tri_has_vert(tri_has_vert, out):
    ...
    ker_key((N_BLOCKS,), (BLOCK_SZ,), (tri_has_vert, tri_has_vert.shape[0], out))
