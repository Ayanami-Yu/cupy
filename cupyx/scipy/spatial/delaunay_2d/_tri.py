...
class GDel2D:
    def __init__(self, points):
        ...
        self.point_vec = cupy.empty((self.n_points, 2), dtype=self.points.dtype)
    ...
    def _init_for_flip(self):
        ...
        get_morton_number(
            self.points, self.n_points - 1, self.min_val, self.range_val, self.values
        )

        self.values[-1] = 2**31 - 1
        unique_values, unique_index = cupy.unique(self.values, return_index=True)
    ...
    def _shift_tri(self, tri_to_vert, split_tri_vec):
        ...
        tri_to_vert = self._shift_replace(shift_vec, tri_to_vert, tri_num, "int")
    ...
    def _expand_tri(self, new_tri_num):
        if new_tri_num > self.tri_num:
            self.triangles = self._expand_copy(self.triangles, (new_tri_num, 3))
            self.triangle_opp = self._expand_copy(self.triangle_opp, (new_tri_num, 3))
    ...
    def _split_tri(self):
        ...
        tri_circle = cupy.full(tri_num, cupy.iinfo(cupy.int32).min, dtype=cupy.int32)
        ...
        tri_to_vert = cupy.full(tri_num, 0x7FFFFFFF, dtype=cupy.int32)
        pick_winner_point(
            self.vert_tri,
            self.n_points,
            vert_circle,
        ...
        if (
            self.available_points - self.ins_num < self.ins_num
            and self.ins_num < 0.1 * self.n_points
        ):
    ...
    def _relocate_all(self):
        ...
        if self.available_points > 0:
            ...
            for i in range(len(self._org_flip_num) - 1, -1, -1):
                ...
                update_flip_trace(self.flip_vec, tri_to_flip, prev_flip_num, flip_num)
        ...
        self._tri_msg = cupy.full((self.max_triangles, 2), -1, dtype=cupy.int32)
        self._tri_msg_sz = 0
        self._org_flip_num = []
    ...
    def _do_flipping(self, check_mode):
        ...
        if (
            check_mode != CheckDelaunayMode.CircleExactOrientSoS
            and org_act_num < 64 * 32
        ):
        ...
        if (
            org_act_num < 512 * 128
            and org_act_num * 2 < self.max_triangles
            and org_act_num * 2 < tri_num
        ):
        ...
        if self._act_tri_mode == ActTriMode.ActTriCollectCompact:
            self._act_tri = self._expand_copy(self._act_tri, org_act_num + flip_num)
    ...
    def _grow_flip_vec(self, size):
        if size > self._flip_vec_cap:
            self._flip_vec = self._expand_copy(self._flip_vec, (size, 4), zeros=True)
    ...
    def _compact_tris(self):
        ...
        compact_tris(
            self.triangle_info, prefix, self.triangles, self.triangle_opp, new_tri_num
        )
    ...
    def _output(self):
        mark_inf_tri(
            self.triangles, self.triangle_info, self.triangle_opp, self.n_points - 1
        )
    ...
    def vertex_neighbor_vertices(self):
        if self.vertex_neighbors is None:
            ...
            edge_enc = cupy.empty(3 * self.triangles.shape[0], dtype=cupy.uint32)
            self.edges = cupy.empty((3 * self.triangles.shape[0], 2), dtype=cupy.int32)
            encode_edges(
                self.triangles,
                self.points,
                self.min_val,
            ...
            vertex_count = cupy.zeros(self.points.shape[0] + 1, dtype=cupy.int32)
            ...
            fill_vertex_neighbors(
                self.edges, self.vertex_off, vertex_count[1:], self.vertex_neighbors
            )
    ...
    def encode_barycenters(self):
        ...
        encode_barycenters(
            self.triangles, self.points, self.min_val, self.range_val, out, centers
        )
        return out, centers
    ...
    def find_point_in_triangulation(self, points, eps=0.0, find_coords=False):
        ...
        if find_coords:
            coords = cupy.empty(
                (points.shape[0], points.shape[-1] + 1), dtype=cupy.float64
            )
