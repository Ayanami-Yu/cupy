...
def make_interp_spline(x, y, k=3, t=None, bc_type=None, axis=0, check_finite=True):
    ...
    if nleft > 0 or nright > 0:
        ...
        interval_kernel(
            (1,), (2,), (t, cupy.r_[x[0], x[-1]], intervals_bc, k, nt, False, 2)
        )
    ...
    if nleft > 0:
        ...
        for j, m in enumerate(deriv_l_ords):
            ...
            )  # the length of the `x0` array
            left = intervals_bc[0]
            rows[j, left - k : left + 1] = temp[: k + 1]

        matr = sparse.vstack([sparse.csr_matrix(rows), matr])  # A[:nleft, :]
    ...
    if nright > 0:
        ...
        matr = sparse.vstack([matr, sparse.csr_matrix(rows)])  # A[nleft+len(x):, :]
    ...
    if cupy.issubdtype(rhs.dtype, cupy.complexfloating):
        ...
        coef = spsolve(matr, rhs.real) + spsolve(matr, rhs.imag) * 1.0j
    else:
...
def _make_interp_spline_full_matrix(x, y, k, t, bc_type):
    ...
    d_boor_kernel(
        ((x.shape[0] + 128 - 1) // 128,),
        (128,),
        (t, dummy_c, k, 0, x, intervals, out, temp, num_c, 0, x.shape[0]),
    )
    ...
    if nleft > 0:
        ...
        for j, m in enumerate(deriv_l_ords):
            ...
            d_boor_kernel(
                (1,),
                (1,),
                (t, dummy_c, k, int(m), x0, intervals_bc, out, temp, num_c, 0, 1),
            )
            left = intervals_bc[0]
...
def _make_periodic_spline(x, y, t, k, axis):
    ...
    matr_csr = sparse.vstack([sparse.csr_matrix(rows), matr])  # A[:nleft, :]
...
def _lsq_solve_qr(x, y, t, k, w):
    ...
    qr_reduce((1,), (1,), (R, m, k + 1, offset, nc, y_w, y_w.shape[1], 1))
