...
def _get_generic_filter_red(
    rk, in_dtype, out_dtype, filter_size, mode, wshape, offsets, cval, int_type
):
    ...
    """.format(
        size=filter_size, out_ctype=out_ctype
    )

    sub_call = """reduction_kernel::{}(sub_in, sub_out);
    y = cast<Y>(val_out);""".format(
        rk.name
    )
...
void {name}({in_const} CArray<{in_ctype}, 1, true, true>& _raw_{in_name},
    ...
    #undef POST_MAP
        ...
        size=filter_size,
        reduce_type=rk.reduce_type,
        reduce_expr=rk.reduce_expr,
...
@_util.memoize(for_each_device=True)
def _get_generic_filter1d(
    rk, length, n_lines, filter_size, origin, mode, cval, in_ctype, out_ctype, int_type
):
    """
    ...
    if mode == "constant":
        ...
        """.format(
                start=start, end=end, in_length=in_length, cval=cval
            ),
        )
    else:
        if length == 1:
