...
def _lu_factor(a, overwrite_a=False, check_finite=True):
    ...
    if not runtime.is_hip and dev_info[0] < 0:
        ...
        raise ValueError(
            "illegal value in %d-th argument of "
            "internal getrf (lu_factor)" % -dev_info[0]
        )
    elif dev_info[0] > 0:
        warn(
            "Diagonal number %d is exactly zero. Singular matrix." % dev_info[0],
            RuntimeWarning,
            stacklevel=2,
        )
...
_kernel_cupy_split_lu = cupy.ElementwiseKernel(
    ...
    """,
    "cupyx_scipy_linalg_split_lu",
    preamble=_device_get_index,
)
