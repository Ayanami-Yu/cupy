...
hipsparse_url = (
    "https://raw.githubusercontent.com/ROCmSoftwarePlatform/"
    ...
    for i, line in enumerate(stubs):
        if i == 3 and not init:
            ...
            if cu_sig == -1 and hip_sig == -1:
                ...
                print(
                    hip_func,
                    "not found in hipSPARSE, maybe not supported?",
                    file=sys.stderr,
                )
                can_map = False
            else:
            ...
            if can_map:
                ...
                hip_stub_h.append(
                    line[: line.find("return") + 6] + " HIPSPARSE_STATUS_NOT_SUPPORTED;"
                )
...
if __name__ == "__main__":
    ...
    stubs = stubs.replace(
        "INCLUDE_GUARD_STUB_CUPY_CUSPARSE_H", "INCLUDE_GUARD_HIP_CUPY_HIPSPARSE_H"
    )
