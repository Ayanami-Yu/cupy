    ...
    return {
        ...
        "assets": {
            "Linux:x86_64": {
                ...
                "filenames": [
                    f"libcudnn{suffix}.so.{public_version}" for suffix in suffix_list
                ],
            },
            "Windows:x86_64": {
                "url": _make_cudnn_url("windows-x86_64", filename_windows),
                "filenames": [
                    f"cudnn{suffix}64_{major_version}.dll" for suffix in suffix_list
                ],
            },
        },
    }
...
def __make_cutensor_record(
    cuda_version, public_version, min_pypi_version, filename_linux, filename_windows
):
    return {
        "cuda": cuda_version,
...
def _make_nccl_record(
    ...
    public_version,
    min_pypi_version,
    filename_linux_x86_64,
    filename_linux_aarch64,
):
    return {
        "cuda": cuda_version,
        "nccl": full_version,
        ...
        "assets": {
            "Linux:x86_64": {
                "url": _make_nccl_url(public_version, filename_linux_x86_64),
                "filenames": ["libnccl.so.{}".format(full_version)],
            },
            "Linux:aarch64": {
                "url": _make_nccl_url(public_version, filename_linux_aarch64),
                "filenames": ["libnccl.so.{}".format(full_version)],
            },
        },
    }
...
def _unpack_archive(filename, extract_dir):
    try:
        ...
        print(
            "The archive format is not supported in your Python "
...
Should be one of {}.""".format(
                str([x["cuda"] for x in lib_records[library]])
            )
        )
    if prefix is None:
...
Remove the directory first if you want to reinstall.""".format(
                destination
            )
        )
...
The current platform ({}) is not supported.""".format(
    ...
    if library == "cudnn":
        print(
            "By downloading and using cuDNN, you accept the terms and"
        ...
        print(
            "By downloading and using cuTENSOR, you accept the terms and"
...
def main(args):
    ...
    if params.action == "install":
        install_lib(params.cuda, params.prefix, params.library, params.arch)
