...
def equal_range(env, exec_policy, first, last, value, comp=None):
    ...
    params = ", ".join([a.code for a in args])
    return _Data(
        f"thrust::equal_range({params})", _cuda_types.Tuple([first.ctype, first.ctype])
    )
...
def inclusive_scan_by_key(
    env, exec_policy, first1, last1, first2, result, binary_pred=None, binary_op=None
):
...
def inner_product(
    env, exec_policy, first1, last1, first2, init, binary_op1=None, binary_op2=None
):
...
def make_constant_iterator(env, x, i=None):
    ...
    params = ", ".join([a.code for a in args])
    return _Data(
        f"thrust::make_constant_iterator({params})", _ConstantIterator(x.ctype)
    )
...
def make_counting_iterator(env, x, i=None):
    ...
    params = ", ".join([a.code for a in args])
    return _Data(
        f"thrust::make_counting_iterator({params})", _CountingIterator(x.ctype)
    )
...
def mismatch(env, exec_policy, first1, last1, first2, pred=None):
    ...
    params = ", ".join([a.code for a in args])
    return _Data(
        f"thrust::mismatch({params})", _cuda_types.Tuple([first1.ctype, first2.ctype])
    )
...
@_wrap_thrust_func(["thrust/unique.h"])
def unique_by_key(
    env, exec_policy, keys_first, keys_last, values_first, binary_pred=None
):
