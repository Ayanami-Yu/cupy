...
class LenFunc(BuiltinFunc):
    ...
    def call(self, env, *args, **kwds):
        ...
        return Data(
            f"static_cast<long long>({arg.code}.shape()[0])", _cuda_types.Scalar("q")
        )
...
class MinFunc(BuiltinFunc):
    ...
    def call(self, env, *args, **kwds):
        ...
        if kwds:
            raise TypeError("keyword arguments are not supported")
        return reduce(
            lambda a, b: _compile._call_ufunc(cupy.minimum, (a, b), None, env), args
        )
...
class MaxFunc(BuiltinFunc):
    ...
    def call(self, env, *args, **kwds):
        ...
        if kwds:
            raise TypeError("keyword arguments are not supported")
        return reduce(
            lambda a, b: _compile._call_ufunc(cupy.maximum, (a, b), None, env), args
        )
        ...
        if op == "CAS":
            ...
            if ctype.dtype.char == "H":
                if int(device.get_compute_capability()) < 70:
                    raise RuntimeError(
                        "uint16 atomic operation is not supported before " "sm_70"
                    )
