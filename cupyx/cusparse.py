...
def csrmvEx(a, x, y=None, alpha=1, beta=0, merge_path=True):
    ...
    algmode = (
        _cusparse.CUSPARSE_ALG_MERGE_PATH
        if merge_path
        else _cusparse.CUSPARSE_ALG_NAIVE
    )
...
def csrsort(x):
    ...
    buffer_size = _cusparse.xcsrsort_bufferSizeExt(
        handle, m, n, nnz, x.indptr.data.ptr, x.indices.data.ptr
    )
...
def coosort(x, sort_by="r"):
    ...
    if sort_by == "r":
        _cusparse.xcoosortByRow(
            handle, m, n, nnz, x.row.data.ptr, x.col.data.ptr, P.data.ptr, buf.data.ptr
        )
    elif sort_by == "c":
...
def dense2csc(x):
    ...
    _call_cusparse(
        ...
        x.dtype,
        handle,
        _cusparse.CUSPARSE_DIRECTION_COLUMN,
        m,
        n,
        descr.descriptor,
        x.data.ptr,
        m,
        nnz_per_col.data.ptr,
        nnz.ctypes.data,
    )
...
def dense2csr(x):
    ...
    _call_cusparse(
        ...
        x.dtype,
        handle,
        _cusparse.CUSPARSE_DIRECTION_ROW,
        m,
        n,
        descr.descriptor,
        x.data.ptr,
        m,
        nnz_per_row.data.ptr,
        nnz.ctypes.data,
    )
    ...
    if _runtime.is_hip:
        if nnz == 0:
            raise ValueError(
                "hipSPARSE currently cannot handle " "sparse matrices with null ptrs"
            )
    data = _cupy.empty(nnz, x.dtype)
    indptr = _cupy.empty(m + 1, "i")
    indices = _cupy.empty(nnz, "i")
    ...
    def create(cls, a):
        ...
        if a.format == "csr":
            desc = _cusparse.createCsr(
                ...
                a.indices.data.ptr,
                a.data.data.ptr,
                _dtype_to_IndexType(a.indptr.dtype),
                _dtype_to_IndexType(a.indices.dtype),
                idx_base,
                cuda_dtype,
            )
            ...
            desc = _cusparse.createCoo(
                ...
                a.col.data.ptr,
                a.data.data.ptr,
                _dtype_to_IndexType(a.row.dtype),
                idx_base,
                cuda_dtype,
            )
            ...
            raise ValueError(
                "csr, csc and coo format are supported "
...
class DnMatDescriptor(BaseDescriptor):
    ...
    def create(cls, a):
        ...
        ld = rows
        cuda_dtype = _dtype.to_cuda_dtype(a.dtype)
        desc = _cusparse.createDnMat(
            rows, cols, ld, a.data.ptr, cuda_dtype, _cusparse.CUSPARSE_ORDER_COL
        )
...
def spmv(a, x, y=None, alpha=1, beta=0, transa=False):
    ...
    if isinstance(a, cupyx.scipy.sparse.csc_matrix):
        ...
        transa = not transa
    if not isinstance(
        a, (cupyx.scipy.sparse.csr_matrix, cupyx.scipy.sparse.coo_matrix)
    ):
        raise TypeError("unsupported type (actual: {})".format(type(a)))
    a_shape = a.shape if not transa else a.shape[::-1]
    if a_shape[1] != len(x):
...
def spmm(a, b, c=None, alpha=1, beta=0, transa=False, transb=False):
    ...
    buff_size = _cusparse.spMM_bufferSize(
        handle,
        op_a,
        op_b,
        alpha.data,
...
def csrsm2(
    ...
    if not (
        cupyx.scipy.sparse.isspmatrix_csr(a) or cupyx.scipy.sparse.isspmatrix_csc(a)
    ):
    ...
    if cupyx.scipy.sparse.isspmatrix_csc(a):
        if transa == _cusparse.CUSPARSE_OPERATION_CONJUGATE_TRANSPOSE:
            raise ValueError(
                "If matrix is CSC format and complex dtype," "transa must not be 'H'"
            )
        a = a.T
...
def denseToSparse(x, format="csr"):
    ...
    handle = _device.get_cusparse_handle()
    buff_size = _cusparse.denseToSparse_bufferSize(
        handle, desc_x.desc, desc_y.desc, algo
    )
    buff = _cupy.empty(buff_size, _cupy.int8)
    _cusparse.denseToSparse_analysis(
        handle, desc_x.desc, desc_y.desc, algo, buff.data.ptr
    )
    num_rows_tmp = _numpy.array(0, dtype="int64")
    num_cols_tmp = _numpy.array(0, dtype="int64")
    nnz = _numpy.array(0, dtype="int64")
    _cusparse.spMatGetSize(
        desc_y.desc, num_rows_tmp.ctypes.data, num_cols_tmp.ctypes.data, nnz.ctypes.data
    )
    nnz = int(nnz)
    if _runtime.is_hip:
        if nnz == 0:
    ...
    desc_y = SpMatDescriptor.create(y)
    _cusparse.denseToSparse_convert(
        handle, desc_x.desc, desc_y.desc, algo, buff.data.ptr
    )
...
def sparseToDense(x, out=None):
    ...
    handle = _device.get_cusparse_handle()
    buff_size = _cusparse.sparseToDense_bufferSize(
        handle, desc_x.desc, desc_out.desc, algo
    )
    buff = _cupy.empty(buff_size, _cupy.int8)
    if _runtime.is_hip:
        if x.nnz == 0:
...
def spgemm(a, b, alpha=1):
    ...
    _cusparse.spMatGetSize(
        mat_c.desc, c_num_rows.ctypes.data, c_num_cols.ctypes.data, c_nnz.ctypes.data
    )
    ...
    _cusparse.csrSetPointers(
        mat_c.desc, c_indptr.data.ptr, c_indices.data.ptr, c_data.data.ptr
    )
