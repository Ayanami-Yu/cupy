...
def _get_functions(obj, exclude=None):
    return set(
        [
            n
            for n, target in [(n, getattr(obj, n)) for n in dir(obj)]
            if (
                # not in exclude list
                ...
                and (
                    ...
                    or not issubclass(target, (BaseException,))
                )
...
def _generate_comparison_rst(
    base_mod, cupy_mod, base_type, klass, exclude_mod, exclude, footnotes=None
):
    ...
    buf += [
        ...
        "   Number of functions covered by CuPy: {}".format(len(cp_funcs & base_funcs)),
...
def generate():
    ...
    buf += _section(
        "Discrete Fourier Transform", "numpy.fft", "cupy.fft", exclude=["test"]
    )
    buf += _section("Random Sampling", "numpy.random", "cupy.random", exclude=["test"])
    buf += _section(
        "Polynomials", "numpy.polynomial", "cupy.polynomial", exclude=["test"]
    )
    buf += _section(
        "Power Series",
